...
skill-library-repository.ts: skills 库 JSON 真相源，负责教学完成产物持久化、skills 列表读取、项目 workflow snapshot -> skill save candidate 恢复与对话模式摘要折叠。
...
- backend 若因重启丢失 session 内存，只允许从项目 `.tesseract/workflow.json` 恢复 save-skill 上下文；不得在其他目录再长出第二份临时 snapshot。
