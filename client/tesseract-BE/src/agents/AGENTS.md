...
config-agent.ts: 工作流创建后的节点配置引导与确认链路；031 起显式拆分软件完成、硬件拼装完成与 actionReady，混合工作流在软件侧完成后进入 hot_plugging 而不是直接 config_complete。
dialogue-mode/: 对话模式技能库、路由与硬件就绪态编排。
hardware-components.ts: 硬件组件目录与别名真相源。
intake-agent.ts: 教学模式入口 facade，把用户输入交给 orchestrator。
mqtt-hardware-runtime.ts: MQTT 硬件运行时真相源，负责 heartbeat 解析、`cmd` 命令封装、raw audio-test / rec_img publish 与 digital twin runtime snapshot。
