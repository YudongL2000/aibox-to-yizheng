/**
 * [INPUT]: 依赖 ./tool-docs 的文档真相源
 * [OUTPUT]: 对外提供工具文档渲染、搜索、分类与 overview 生成函数
 * [POS]: mcp 的文档投影层，被 tools_documentation 工具直接消费
...
2. **Configure** the node (ALWAYS START WITH STANDARD DETAIL):
   ...
   - get_node_docs({nodeType: "nodes-base.slack"}) - Get readable markdown documentation
   - search_node_properties({nodeType: "nodes-base.slack", query: "auth"}) - Find specific properties
   - get_node_versions({nodeType: "nodes-base.slack", since: "1.0"}) - Review version changes when upgrading
...
- get_node - Node schema and required fields with progressive detail levels
- get_node_docs - Readable markdown documentation for one node
- search_node_properties - Property lookup inside a node schema
- get_node_versions - Version history and upgrade analysis
...
**n8n API Tools** (11 tools, requires N8N_API_URL configuration)
...
- Instant (<10ms): search_nodes, get_node (minimal/standard), get_node_docs, search_node_properties, get_node_versions
- Fast (<100ms): validate_node
...
- Use get_node_docs/search_node_properties/get_node_versions for docs, field lookup, and upgrade questions instead of overloading get_node
