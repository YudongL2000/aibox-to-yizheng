/*
 * [INPUT]: 依赖 ngx-x-markdown 的 XMarkdownComponent/StreamingOption/ComponentMap，注入 AilyChatCodeComponent 作为代码块渲染器。
 * [OUTPUT]: 对外提供 XDialogComponent，将 AI 消息内容预处理后交给 x-markdown 渲染。
 * [POS]: x-dialog 目录的顶层消息气泡组件；preprocess() 是所有 AI 文本进入渲染管线的唯一入口。
...
export class XDialogComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['doing'] || changes['content']) {
      ...
      // 流式结束时重新预处理（确保 filterToolCalls 状态最终化）
      if (!this.doing) {
  ...
  /**
   * aily-mermaid 块预处理（pass-through）
   *
   * JSON 包装已移除：
   * - ngx-x-markdown 渲染 fenced block 时会 unescape \n → 真实换行、\" → "，
   *   导致 JSON.stringify 产生的字符串在传入下游时变为无效 JSON。
   * - AilyChatCodeComponent.mermaidData 已通过 regex/raw fallback 正确处理原始 mermaid 代码，
   *   无需此处做任何包装转换。
   */
  ...
  private filterToolCalls(content: string): string {
    ...
    for (const line of content.split('\n')) {
      ...
      if (
        json.type === 'ToolCallExecutionEvent' &&
  ...
  private extractContextLabel(text: string): string {
    ...
    if (text.includes('参考文件:')) {
      const n =
        text.split('参考文件:')[1]?.split('\n\n')[0]?.match(/^- /gm)?.length ??
        0;
      if (n > 0) parts.push(`${n}个文件`);
    }
    if (text.includes('参考文件夹:')) {
      const n =
        text.split('参考文件夹:')[1]?.split('\n\n')[0]?.match(/^- /gm)
          ?.length ?? 0;
  ...
  private fixContent(content: string): string {
    ...
    const isValidLang = (s: string) =>
      /^[a-zA-Z0-9+#_.-]+$/.test(s.trim()) && s.trim().length > 0;
    content = content.replace(/```([^\n`]*)/g, (match, after) => {
      if (ailyTypes.some((t) => after.startsWith(t) || t.startsWith(after)))
        return match;
    ...
    return content
      ...
      .replace(
        /\s*```(aily-(?:board|library|state|button|task-action|think|context|workflow-blueprint|component-recommendation|config-guide))/g,
        '\n```$1\n',
      );
  }
