 ...
 * [OUTPUT]: 对外提供 DigitalTwinConsoleController，统一维护场景、灯光、选中项、组件缩放、组件安装锚点、接口-组件残差与 viewer 同步逻辑，并把场景位姿收敛为预览窗口全局坐标系的绝对值；默认 viewer 背景回退继续跟随当前 dark shell。
...
class DigitalTwinConsoleController extends ChangeNotifier {
  ...
  String normalizeHexColor(String value, {String? fallback}) {
    final normalized = value.trim().replaceFirst('#', '').toUpperCase();
    if (!RegExp(r'^[0-9A-F]{6}$').hasMatch(normalized)) {
      return fallback ?? '#${SpatialDesignTokens.darkBgBaseHex}';
    }
    return '#$normalized';
  }
  ...
  Color parseHexColor(String value) {
    ...
    if (parsed == null) return SpatialDesignTokens.darkBgBase;
    return Color(parsed);
  }
