...
export class AilyMermaidViewerComponent
  implements OnInit, OnDestroy, OnChanges
{
  ...
  private emitLog(
    ...
    const logger =
      typeof window !== 'undefined' ? window['electronAPI']?.log : null;
  ...
  private processData(): void {
    ...
    try {
      // 提取 Mermaid 代码
      let code = this.resolveMermaidSource(
        this.data.code || this.data.content || this.data.raw || '',
      );
  ...
  private notifyMermaidReady(diagramId: string): void {
    try {
      ...
      this.emitLog(
        'warn',
        '[AilyMermaid] Mermaid ready 事件派发失败',
        this.describeError(error),
      );
    }
  }
