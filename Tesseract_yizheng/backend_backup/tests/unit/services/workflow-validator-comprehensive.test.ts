...
describe('WorkflowValidator - Comprehensive Tests', () => {
  ...
  describe('validateWorkflow', () => {
    it('should validate a minimal valid workflow', async () => {
      const workflow = createWorkflow('Test Workflow')
        ...
        .addCodeNode({ id: 'code-1', name: 'Process Data' })
        .build();
  ...
  describe('validateWorkflowStructure', () => {
    ...
    it('should error for single non-webhook node workflow', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.message.includes('Single-node workflows are invalid for Tesseract'))).toBe(true);
    });

    it('should reject webhook without connections', async () => {
      const workflow = {
        nodes: [{
          id: '1',
          name: 'Webhook',
