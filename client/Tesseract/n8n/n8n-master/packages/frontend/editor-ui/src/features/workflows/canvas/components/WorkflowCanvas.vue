...
// 加载完成后自动整理布局并适配视口，避免首次渲染卡片错乱需手动点击「整理」按钮
const { off } = onNodesInitialized(() => {
	if (!initialLayoutDone.value) {
		initialLayoutDone.value = true;
		off();
		// 延迟执行，确保自定义卡片节点已渲染完成再计算布局
		setTimeout(() => {
			props.eventBus.emit('tidyUp', {
				source: 'canvas-initial-load',
				trackEvents: false,
				trackHistory: false,
			});
		}, 200);
...
.canvas {
	...
	--canvas--dot--color: var(--color--neutral-600);
}
</style>
