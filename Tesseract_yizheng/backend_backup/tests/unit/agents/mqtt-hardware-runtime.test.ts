 ...
 * [OUTPUT]: 对外提供 MQTT 硬件运行时解析、映射与命令封装回归测试
...
describe('MqttHardwareRuntime', () => {
  ...
  it('parses status/msg_content heartbeat payloads and preserves mapped ports from port field', () => {
    const record = parseHeartbeatPayload({
      msg_type: 'status',
      timestamp: 789,
      msg_content: [
        {
  ...
  it('falls back to canonical alias ports when heartbeat omits explicit port fields', () => {
    const runtime = new MqttHardwareRuntime({
      enabled: false,
      broker: '127.0.0.1',
      port: 1883,
