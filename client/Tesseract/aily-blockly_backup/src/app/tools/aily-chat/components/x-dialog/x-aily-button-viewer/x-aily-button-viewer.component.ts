 ...
 * [OUTPUT]: 对外提供 x-aily-button-viewer，渲染操作按钮并为真实硬件端口确认补齐接口选择。
...
@Component({
          ...
          <div class="ac-btn-wrap">
            @if (isHotplugButton(btn)) {
              <div class="port-picker">
                <div class="port-picker__title">{{ btn.payload?.portSelectionTitle || '选择接口' }}</div>
  ...
  styles: [`
    ...
    .ac-btn-wrap { display: grid; gap: 10px; }
    .ac-btn {
      align-items: center;
      border-radius: 6px;
      cursor: pointer;
    ...
    .port-chip:hover:not(:disabled) {
      transform: translateY(-1px);
      border-color: rgba(173, 117, 255, 0.45);
    }

    .port-chip.active {
      background: rgba(141, 84, 255, 0.18);
      border-color: rgba(173, 117, 255, 0.72);
      color: #ffffff;
    }
  `],
})
export class XAilyButtonViewerComponent implements OnChanges {
  ...
  isHotplugButton(btn: ButtonData): boolean {
    return btn.action === 'tesseract-confirm-node' && this.getPortOptions(btn).length > 0;
  }
