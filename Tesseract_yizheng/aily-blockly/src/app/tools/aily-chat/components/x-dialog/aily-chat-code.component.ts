/*
 * [INPUT]: 依赖 aily-* markdown code block 协议与各 viewer 组件。
 * [OUTPUT]: 对外提供 AilyChatCodeComponent，负责把 markdown block 分发到对应的 aily-* viewer。
 *           mermaidData getter 支持三条路径：parsedData.code（正常 JSON）、regex 提取（旧式 JSON 包装被 markdown unescape 破坏）、raw fallback（原始 mermaid）。
 * [POS]: x-dialog 的代码块路由器，是聊天消息到业务卡片的唯一渲染分发层。
...
import { XAilyWorkflowBlueprintViewerComponent } from './x-aily-workflow-blueprint-viewer/x-aily-workflow-blueprint-viewer.component';
import { XAilyComponentRecommendationViewerComponent } from './x-aily-component-recommendation-viewer/x-aily-component-recommendation-viewer.component';
import { XAilyConfigGuideViewerComponent } from './x-aily-config-guide-viewer/x-aily-config-guide-viewer.component';
import { XAilyDialogueModeViewerComponent } from './x-aily-dialogue-mode-viewer/x-aily-dialogue-mode-viewer.component';
...
@Component({
  ...
  imports: [
    ...
    XAilyWorkflowBlueprintViewerComponent,
    XAilyComponentRecommendationViewerComponent,
    XAilyConfigGuideViewerComponent,
    XAilyDialogueModeViewerComponent,
  ],
  ...
  template: `
    ...
    @if (isType('aily-mermaid') || isMermaidStd) {
      <div
        class="aily-mermaid-clickable"
        (click)="openMermaidFullscreen()"
        title="点击全屏查看"
      >
        <ngx-mermaid-code
    ...
    @if (isType('aily-workflow-blueprint') && parsedData) {
      <x-aily-workflow-blueprint-viewer [data]="parsedData" />
    }
    @if (isType('aily-component-recommendation') && parsedData) {
      <x-aily-component-recommendation-viewer [data]="parsedData" />
    }
    @if (isType('aily-config-guide') && parsedData) {
      <x-aily-config-guide-viewer [data]="parsedData" />
    }
    @if (isType('aily-dialogue-mode') && parsedData) {
      <x-aily-dialogue-mode-viewer [data]="parsedData" />
    }
    @if (isRegularCode) {
...
export class AilyChatCodeComponent implements OnChanges, OnDestroy {
  ...
  get mermaidData(): { code?: string } | null {
    ...
    if (this.isType('aily-mermaid')) {
      if (this.parsedData && typeof this.parsedData.code === 'string')
        return this.parsedData;
      const raw = this.decodeEntities(this.children).trim();
      if (!raw) return null;
      // 兜底：若 raw 为 JSON 包装的 mermaid（可能因 markdown renderer un-escape 导致 JSON.parse 失败）
      // 用 regex 提取 code 字段，并还原 escape 序列
      if (raw.startsWith('{')) {
  ...
  async openMermaidFullscreen(): Promise<void> {
    ...
    try {
      ...
      const svg =
        typeof result === 'object' && result?.svg
          ? result.svg
