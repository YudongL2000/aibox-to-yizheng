...
成员清单
aily-chat/: AI 助手主模块，负责旧 Agent/QA 与 Tesseract backend 的教学/对话双子模式切换，并以嵌入形态挂到 `Assistant` 支持层。
skill-center/: Skill 资产模块，负责右侧轻量 Skills 调用态与完整 Skill Center 管理态两种展示。
...
- Tesseract 相关交互应优先在 `aily-chat/` 容器内闭环；但右侧整体布局语义应由 `SupportPanel` 收口成 `Assistant / Assets / Runtime`，不要再把 renderer 业务状态分裂到其他前端壳里。
