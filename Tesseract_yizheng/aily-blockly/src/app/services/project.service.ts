...
export class ProjectService {
  ...
  async projectNew(newProjectData: NewProjectData) {
    try {
      ...
      this.tesseractProjectService.ensureProjectScaffold(projectPath, {
        projectName: newProjectData.name,
        board: newProjectData.board,
        devmode: newProjectData.devmode,
      });
  ...
  async projectOpen(projectPath = this.currentProjectPath) {
    ...
    const mode = this.tesseractProjectService.getProjectMode(projectPath);
    const targetRoute = mode === 'tesseract'
  ...
  async close() {
    ...
    this.router.navigate(['/main/tesseract-studio'], { replaceUrl: true });
  }
  ...
  async getBoardModule() {
    ...
    return Object.keys(dependencies).find(dep => dep.startsWith('@aily-project/board-')) || null;
  }
