 ...
 * [OUTPUT]: 对外提供 registerWindowHandlers(返回 { preloadDigitalTwin })、terminateAilyProcess，并统一维护子窗口集合、初始化数据注入与数字孪生场景广播。
...
function registerWindowHandlers(mainWindow) {
    ...
    // ── 数字孪生窗口 hide / show ──
    ipcMain.handle("digital-twin-hide", async () => {
        forEachOpenWindow((subWindow, _url, entry) => {
            if (entry.windowRole === 'digital-twin' && subWindow && !subWindow.isDestroyed()) {
                subWindow.hide();
            }
        });
    });

    ipcMain.handle("digital-twin-show", async () => {
        forEachOpenWindow((subWindow, _url, entry) => {
            if (entry.windowRole === 'digital-twin' && subWindow && !subWindow.isDestroyed()) {
                subWindow.show();
            }
    ...
    function resolveDigitalTwinBaseUrl() {
        const envKeys = ['AILY_TESSERACT_FRONTEND_URL', 'TESSERACT_FRONTEND_URL'];
        for (const key of envKeys) {
            const value = process.env[key]?.trim();
            if (value) {
                try {
                    return buildDigitalTwinFullUrl(value);
                } catch (e) {
                    console.warn(`[DigitalTwin][Preload] env ${key} failed:`, e.message);
                }
            }
        }
        return buildDigitalTwinFullUrl('http://127.0.0.1:18082/index.html');
    }

    function buildDigitalTwinFullUrl(baseUrl) {
    ...
    function preloadDigitalTwin() {
        try {
            const digitalTwinUrl = resolveDigitalTwinBaseUrl();
            const windowPath = `iframe?url=${encodeURIComponent(digitalTwinUrl)}`;

            if (openWindows.has(windowPath)) {
                console.info('[DigitalTwin][Preload] already in openWindows, skip');
                return;
            }

            console.info('[DigitalTwin][Preload] creating hidden window', { windowPath });
            ...
            subWindow.webContents.on('did-finish-load', () => {
                console.info('[DigitalTwin][Preload] did-finish-load, seeding init data');
                subWindow.webContents.send('window-init-data', {
                    url: undefined,
                    title: '数字孪生',
                    data: undefined,
                });
                if (latestDigitalTwinEnvelope) {
                    subWindow.webContents.send('digital-twin-scene-updated', latestDigitalTwinEnvelope);
                }
                if (latestDigitalTwinPreviewStateEnvelope) {
                    subWindow.webContents.send('digital-twin-preview-state-updated', latestDigitalTwinPreviewStateEnvelope);
                }
            });

            subWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
                if (!isMainFrame || errorCode === -3) {
                    return;
                }

                console.error('[DigitalTwin][Preload] failed to load hidden window', {
                    errorCode,
                    errorDescription,
                    validatedURL,
                });
                openWindows.delete(windowPath);
            ...
            if (process.env.DEV === 'true' || process.env.DEV === true) {
                subWindow.loadURL(buildDevServerUrl(`#/${windowPath}`));
            } else {
                subWindow.loadFile(resolveRendererIndexPath(__dirname), { hash: `#/${windowPath}` });
            }

            console.info('[DigitalTwin][Preload] hidden window queued');
        } catch (error) {
            console.error('[DigitalTwin][Preload] failed:', error.message);
        }
    }

    return { preloadDigitalTwin };
}
