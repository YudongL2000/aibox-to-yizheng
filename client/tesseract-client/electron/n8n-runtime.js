...
class N8nRuntime {
  ...
  async runCliCommand(args, logPrefix) {
    ...
    return await new Promise((resolve, reject) => {
      const lines = [];
      const child = spawn(process.execPath, [runtime.bootstrapPath, ...args], {
        cwd: runtime.cliRoot,
        env,
  ...
  resolveN8nRoot() {
    ...
    if (
      ...
      throw new Error(
        "n8n runtime not found. Expected ../n8n/n8n-master/packages/cli/dist. Build the n8n workspace first."
      );
    }
    return { root, cliRoot, entry, bootstrapPath };
  }
  ...
  async start(options = {}) {
    ...
    this.lastError = "";
    this.publicApiEnabled = false;
    this.process = spawn(process.execPath, [runtime.bootstrapPath, "start"], {
      cwd: runtime.cliRoot,
      env,
      stdio: ["ignore", "pipe", "pipe"],
      windowsHide: true,
