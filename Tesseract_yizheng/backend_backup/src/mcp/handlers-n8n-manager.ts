...
const testWorkflowSchema = z.object({
  workflowId: z.string(),
  triggerType: z.enum(['webhook']).optional(),
...
export async function handleTestWorkflow(args: unknown, context?: InstanceContext): Promise<McpToolResponse> {
  try {
    ...
    if (input.triggerType && input.triggerType !== 'webhook') {
      return {
        success: false,
        error: `Unsupported trigger type: ${input.triggerType}`,
        details: {
          hint: 'Only webhook triggers are supported in this build.'
        }
      };
    }
    ...
    if (!hasWebhookTrigger(workflow)) {
      return {
        ...
        details: {
          ...
          hint: 'Add a webhook trigger node or provide webhookPath explicitly.'
        }
      };
    }

    const webhookPath = input.webhookPath || getWebhookUrl(workflow);
    if (!webhookPath) {
      return {
        success: false,
        ...
        details: {
          workflowId: input.workflowId,
          hint: 'Set a webhook path in the webhook node or pass webhookPath.'
        }
      };
    }

    const apiConfig = (context ? getN8nApiConfigFromContext(context) : null) ?? getN8nApiConfig();
    const baseUrl = process.env.N8N_PUBLIC_URL || apiConfig?.baseUrl?.replace(/\/api\/v\d+\/?$/, '');
    if (!baseUrl) {
      return {
        success: false,
        error: 'N8N_PUBLIC_URL is required to trigger webhooks',
        details: {
          hint: 'Set N8N_PUBLIC_URL to your instance base URL (e.g. https://n8n.example.com).'
        }
      };
    }

    const normalizedPath = webhookPath.startsWith('/') ? webhookPath : `/${webhookPath}`;
    const webhookUrl = new URL(normalizedPath, baseUrl.endsWith('/') ? baseUrl : `${baseUrl}/`).toString();
...
export async function handleGetExecution(args: unknown, context?: InstanceContext): Promise<McpToolResponse> {
  try {
    ...
    const result = (effectiveMode || nodeNames || itemsLimit !== undefined)
      ? { ...execution, _note: 'Execution filtering is disabled in this build.' }
      : execution;
...
export async function handleHealthCheck(context?: InstanceContext): Promise<McpToolResponse> {
  ...
  try {
    ...
    const versionCheck = {
      ...
      latestVersion: null,
      isOutdated: false,
      updateCommand: undefined as string | undefined
    };
...
// Diagnostic mode handler for n8n_health_check
export async function handleDiagnostic(request: any, context?: InstanceContext): Promise<McpToolResponse> {
  ...
  const managementTools = apiConfigured ? 11 : 0; // Management tools requiring API
  const totalTools = documentationTools + managementTools;

  const versionCheck = {
    currentVersion: require('../../package.json').version as string,
      ...
      whatYouCanDoNow: {
        documentation: [
          ...
          },
          {
            ...
            example: 'get_node({nodeType: "nodes-base.httpRequest"})'
          },
          {
        ...
        note: '5 documentation tools available without API configuration'
      },
      whatYouCannotDo: [
