...
`src/spatial-design-ref.css` 是 agent-ui dark-first shell 的唯一视觉真相源；Header / AIHealthLab / ChatInterface / InteractionCard / N8nIframe / BuildProgressBar 必须优先消费 semantic panel/chip/button/input/list utilities，不再直接把 Tailwind 颜色类当设计系统。
...
2026-04-12: agent-ui 第一波切到 Spatial Wireframe dark shell；全局 token 扩展为 semantic surface/border/text/status/button/input/list utilities，运行时 tone class 也统一改为语义映射。
