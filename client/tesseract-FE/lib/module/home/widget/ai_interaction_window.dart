 ...
 * [OUTPUT]: 对外提供 AiInteractionWindow 双模组件，统一承接教学模式与 OpenClaw 对话模式，并向父层回传 workflow URL 与 backend-first digital twin scene。
 * [POS]: module/home/widget 的 AI 交互总壳，负责把 backend 响应折叠为工作流卡片或对话模式状态，同时转发物理 cue、部署确认与教学跳转。
...
class _AiInteractionWindowState extends State<AiInteractionWindow> {
  ...
  DialogueModePanelState _dialoguePanelState =
      DialogueModePanelState.initial();
  ...
  bool _shouldValidateDialogueHardware(HardwareBridgeEvent event) {
    ...
    return event.eventType != dialogue_api.DialogueModeHardwareEventType.heartbeat;
  }
    ...
    setState(() {
      ...
      switch (responseData.type) {
        ...
        case ResponseType.configInput:
          ...
          _configProgress = responseData.progress;
        ...
        case ResponseType.configComplete:
          ...
          if ((responseData.progress != null &&
            ...
            _interactionState = InteractionState.deployment;
            _currentStep = 4;
            // _handleConfigComplete(responseData.configMetadata?.workflowId);
            return;
          }
  ...
  void _processCommand(String command) {
    // 根据命令处理不同的交互流程
    if (command.contains('确认') || command.contains('构建')) {
      setState(() {
  ...
  void _handleImagePick() async {
    try {
      ...
      setState(() {
        ...
        _interactionState = InteractionState.imageSelected; // 新状态：图片已选择，等待确认
      });
    } catch (e) {
  ...
  Widget _buildBlueprintSummary(double buttonMaxWidth) {
    ...
    return Stack(
      ...
      children: [
        card,
        Positioned.fill(child: _buildCardLoadingMask('创作中', borderRadius: 36)),
      ],
    );
  }

  /// 构建卡片上的加载蒙板（半透明 + 中间文案）
  Widget _buildCardLoadingMask(String text, {double borderRadius = 36}) {
    return Container(
  ...
  /// 构建工作流确认组件（创建中显示蒙板「创作中」）
  Widget _buildWorkflowConfirm() {
    if (_currentWorkflow == null) return const SizedBox.shrink();

    const workflowCardRadius = 8.0;
    final card = WorkflowConfirmWidget(
      workflowName: _currentWorkflow!.name,
      nodeCount: _workflowData?['nodeCount'] != null
          ? int.tryParse(_workflowData!['nodeCount'].toString())
          : null,
      workflowData: {'id': _currentWorkflow!.name},
      onCancel: () {
        setState(() {
          _interactionState = InteractionState.idle;
        });
    ...
    if (!_isCreatingWorkflow) return card;
    ...
    return Stack(
      ...
      children: [
        card,
        Positioned.fill(
          child: _buildCardLoadingMask('创作中', borderRadius: workflowCardRadius),
        ),
      ],
    );
  }
  ...
  bool _isInteractiveType(String type) {
    // 根据实际需求判断哪些类型是交互式的
    return type.contains('interactive') || type.contains('select');
  }
  ...
  /// 构建配置交互界面
  Widget _buildConfigInteraction() {
    if (_currentConfigNode == null) return const SizedBox.shrink();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            _currentConfigNode!.title ??
                _currentConfigNode!.displayName ??
                '配置交互',
            style: const TextStyle(
              color: Colors.white,
              fontSize: 16,
          ...
          if (_currentConfigNode!.subtitle != null) ...[
            const SizedBox(height: 8),
            Text(
              _currentConfigNode!.subtitle!,
              style: TextStyle(
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  onPressed: () => _confirmNodeWithInteraction(
  ...
  /// 显示配置确认按钮
  void _showConfigConfirmButton(ResponseData response) {
    setState(() {
      _messages.add(
        InteractionMessage(
          ...
          isUser: false,
          type: MessageType.instruction,
          data: {'showConfigConfirm': true, 'configData': response},
        ),
      );
    });
  }
  ...
  /// 配置完成处理（请求 n8n 获取工作流并发送 MQTT）
  Future<void> _handleConfigComplete(String? workflowId) async {
    if (workflowId == null) {
      EasyLoading.showError('工作流ID不存在');
      setState(() {
        _isConfiguring = false;
      });
      return;
    ...
    try {
      EasyLoading.show(status: '获取工作流数据...');
      await _fetchWorkflowAndSendMqtt(workflowId);
      EasyLoading.dismiss();
      ...
      setState(() {
        _messages.add(
          InteractionMessage(
            text: '配置已完成，但部署未完成：$errorMessage',
            isUser: false,
            type: MessageType.system,
          ),
        );
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: RadialGradient(
          ...
          radius: 1.0,
          colors: [const Color(0xFF0A1930), const Color(0xFF010409)],
        ),
          ...
          children: [
            Column(
              ...
              children: [
                ...
                Text(
                  isDialogue ? 'OpenClaw Dialogue Mode' : 'Terminal Hub',
                  style: TextStyle(
  ...
  Widget _buildHeaderIcon(
    ...
    final borderRadius = useN8nStyle ? 6.0 : 12.0;
    ...
    final iconColor = useN8nStyle
        ...
        : (isPrimary ? const Color(0xFF00F2FF) : Colors.white.withOpacity(0.7));
              ...
              children: [
                Expanded(
                  child: Container(
                    height: 1,
                    decoration: BoxDecoration(
                      color: lineActive ? cyan : cyan.withOpacity(0.1),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isActive ? cyan : cyan.withOpacity(0.2),
                      width: 2,
                    ),
                    boxShadow: isActive
                        ? [
                  ...
                  child: Center(
                    child: Text(
                      '$step',
                      style: TextStyle(
                        color: isActive ? cyan : Colors.white.withOpacity(0.5),
                        fontSize: 11,
                        fontWeight: FontWeight.w600,
                      ),
                    ),
                    ...
                    child: Row(
                      ...
                      children: [
                        ConstrainedBox(
                          ...
                          child: UserInputBubble(text: _dialogueLastUserPrompt!),
                        ),
                      ],
                    ),
                  ),
                    ...
                    child: ConstrainedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: (_isConfiguring || !widget.isN8nLoaded)
                            ...
                            : _startConfig,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: widget.isN8nLoaded
                              ? const Color(0xFF00D9FF)
  ...
  // 底部输入区域（参考图1：inspiration-pill + 圆角输入框 + THINKING + SEND）
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              //       onTap: () {
              //         setState(() {
              //           _isVoiceBroadcastEnabled = !_isVoiceBroadcastEnabled;
              //         });
              //       },
              //       child: Container(
              //         width: 44,
              //         height: 24,
              //         decoration: BoxDecoration(
              //           color: _isVoiceBroadcastEnabled
              //               ? const Color(0xFF00D9FF)
              ...
              //               left: _isVoiceBroadcastEnabled ? 22 : 2,
              //               top: 2,
              //               child: Container(
              //                 width: 20,
              //                 height: 20,
                        ...
                        Padding(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              hintText: _panelMode == _AssistantPanelMode.dialogue
                                  ? '输入指令调用 Skill 或学习新需求...'
                                  : '输入下一步指令...',
                              hintStyle: TextStyle(
  ...
  List<String> get _activeSuggestionTexts => _panelMode == _AssistantPanelMode.dialogue
  ...
  Widget _buildModeToggleBar() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: const Color(0xFFB05CFF).withOpacity(0.22),
          width: 1,
        ),
      ),
      child: Row(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: Text(
            label,
            style: TextStyle(
              color: Colors.white.withOpacity(selected ? 0.98 : 0.72),
              fontSize: 13,
  ...
  Widget _buildDialogueBridgeIndicator() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isConnected
              ? const Color(0xFF7BFFB0).withOpacity(0.35)
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isConnected
                ? const Color(0xFF7BFFB0).withOpacity(0.32)
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                shape: BoxShape.circle,
                boxShadow: [
                  BoxShadow(color: cyan.withOpacity(0.6), blurRadius: 8),
                ],
              ),
            ),
            const SizedBox(width: 8),
