		...
		: []),
	{
		...
		transformIndexHtml: (html, ctx) => {
			...
			return ctx.server
				? html
						.replace('%CONFIG_TAGS%', '')
						.replaceAll('/{{BASE_PATH}}', 'http://118.196.33.248:40005')
						.replaceAll('/{{REST_ENDPOINT}}', '/rest')
				: html;
		},
		configureServer(server) {
			// 处理浏览器缓存的旧资源路径，重定向到开发服务器的入口文件
			server.middlewares.use((req, res, next) => {
				if (req.url?.startsWith('/assets/') && req.url.includes('.js')) {
					res.writeHead(302, { Location: '/src/main.ts' });
					res.end();
					return;
				}
				next();
...
export default mergeConfig(
	defineConfig({
		...
		server: {
			...
			proxy: {
				'/rest': {
					target: 'http://118.196.33.248:40005',
					changeOrigin: true,
					secure: false,
					ws: true,
					timeout: 60000,
