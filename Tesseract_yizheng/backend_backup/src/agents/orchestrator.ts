 ...
 * [OUTPUT]: 对外提供 Orchestrator 多轮编排入口（process/confirm）与 AI 驱动的语义发现/澄清动作生成，并在 summary 已允许继续时兜底最小工作流确认链路
  ...
  async confirm(sessionId: string): Promise<AgentResponse> {
    ...
    if (capabilities.length === 0) {
      this.emitTrace(sessionId, {
        ...
        status: 'fallback',
        title: '确认构建回退为最小工作流',
        detail: 'summary 阶段已允许继续，但显式 capability 为空；继续走 WorkflowArchitect/ComponentComposer 兜底生成',
        data: {
