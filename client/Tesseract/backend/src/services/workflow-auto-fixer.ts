/**
 * [INPUT]: 依赖 workflow-validator 的错误元数据、expression-format issue 与 diff operation 协议
 * [OUTPUT]: 对外提供 WorkflowAutoFixer 与常见工作流错误的自动修复操作生成
 * [POS]: services 的工作流自动修复层，承接 unified-validator 判定为 autoFixable 的问题
...
export class WorkflowAutoFixer {
  ...
  async generateFixes(
    ...
    if (!fullConfig.fixTypes || fullConfig.fixTypes.includes('code-node-template')) {
      this.processCodeNodeFixes(validationResult, nodeMap, operations, fixes);
    }
  ...
  private processCodeNodeFixes(
    ...
    for (const error of validationResult.errors) {
      ...
      const after = buildExecutableCodeNodeParameters(
      ...
      fixes.push({
        node: nodeName,
        field: 'parameters',
        type: 'code-node-template',
        before,
        after,
        confidence: 'medium',
        description: '为 code 节点补齐最小可执行模板，避免空代码被校验器拒绝',
      });
  ...
  private generateSummary(stats: AutoFixResult['stats']): string {
    ...
    if (stats.byType['code-node-template'] > 0) {
      parts.push(`${stats.byType['code-node-template']} code node ${stats.byType['code-node-template'] === 1 ? 'template' : 'templates'}`);
    }
