  ...
  template: `
    <div class="guide-card">
      ...
      @if (isHotPlugging) {
        <div class="hotplug-status-card">
          <div class="hotplug-status">
            ...
            <span class="hotplug-counter"
              >{{ hotplugConnectedCount }}/{{ requiredComponents.length }}</span
            >
          </div>
          ...
          @if (requiredComponents.length > 0) {
            <div class="hotplug-checklist">
              @for (
                component of requiredComponents;
                track component.componentId
              ) {
                <div
                  class="hotplug-item"
                  [class.ready]="isComponentDetected(component)"
                >
          ...
          @if (hotplugTimedOut && !allRequiredDetected) {
            <div class="hotplug-timeout">
              等待超过 30
              秒。请检查硬件连接，或点击下方“标记节点已处理”先跳过当前步骤。
            </div>
          }
        </div>
      }
      ...
      @if (showSelectOptions) {
        <div class="options">
          @for (
            option of asArray(data?.interaction?.options);
            track option.value
          ) {
            <button
          ...
          @if (asArray(data?.interaction?.options).length) {
            <div class="upload-group">
              ...
              <span class="upload-profile-text">{{
                asArray(data?.interaction?.options)[0]?.label || '目标人物'
              }}</span>
            </div>
          }
  ...
  styles: [
    `
      .guide-card {
        ...
        border: 1px solid
          color-mix(in srgb, var(--accent) 22%, var(--border-subtle));
      ...
      .hotplug-item.ready {
        border-color: color-mix(in srgb, var(--sem-success) 28%, transparent);
      ...
      .hotplug-item.ready .hotplug-item-indicator {
        ...
        box-shadow: 0 0 0 4px
          color-mix(in srgb, var(--sem-success) 12%, transparent);
      }
      .hotplug-item-copy {
        display: grid;
        gap: 2px;
      ...
      .hotplug-timeout {
        ...
        border-radius: var(--radius-sm);
        border: 1px solid
          color-mix(in srgb, var(--sem-warning) 24%, transparent);
        background: color-mix(
          in srgb,
          var(--sem-warning) 10%,
          var(--surface-panel)
...
export class XAilyConfigGuideViewerComponent implements OnChanges, OnDestroy {
  ...
  constructor(
    private readonly dialogueHardwareBridgeService: DialogueHardwareBridgeService,
  ) {
    this.hotplugSubscription =
      this.dialogueHardwareBridgeService.state$.subscribe((state) => {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['data']) {
      ...
      this.detectedComponents =
        this.dialogueHardwareBridgeService.snapshot.components;
  ...
  get requiredComponents(): HotplugRequirement[] {
    ...
    const metadataRequirements = Array.isArray(
    ...
    const fallbackName =
      this.data?.currentNode?.displayName ||
      this.data?.currentNode?.title ||
      '当前硬件';
    return [
      {
        componentId: String(fallbackName)
          .trim()
          .toLowerCase()
          .replace(/\s+/g, '_'),
  ...
  get hotplugConnectedCount(): number {
    return this.requiredComponents.filter((component) =>
      this.isComponentDetected(component),
    ).length;
  }

  get allRequiredDetected(): boolean {
    return (
      this.requiredComponents.length > 0 &&
  ...
  get showSelectOptions(): boolean {
    ...
    const hasSelectableInteraction =
      this.asArray(this.data?.interaction?.options).length > 0 &&
      (this.data?.interaction?.mode === 'single' ||
        this.data?.interaction?.mode === 'multi');

    return (
      hasSelectableInteraction ||
  ...
  get showInlineSubmit(): boolean {
    return this.showSelectOptions || this.isConfigInput;
  }
  ...
  get inputPlaceholder(): string {
    return (
      this.data?.currentNode?.subtitle ||
      this.data?.interaction?.description ||
      '输入配置内容'
    );
  }
  ...
  submit(): void {
    ...
    if (this.showSelectOptions) {
      ...
      this.dispatchAction('tesseract-submit-config-interaction', {
        ...
        displayValue,
        clarificationCategory: (selectedOption as any)?.category || null,
      });
      return;
    }
  ...
  private resetStateFromData(): void {
    ...
    this.selectedProfile =
      typeof selected === 'string'
        ? selected
        : this.asArray(this.data?.interaction?.options)[0]?.value || '';
    this.selectedFileName = '';
    this.selectedFileBase64 = '';
  }
  ...
  private readFileAsBase64(file: File): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        resolve(
          separatorIndex >= 0 ? result.slice(separatorIndex + 1) : result,
        );
      };
