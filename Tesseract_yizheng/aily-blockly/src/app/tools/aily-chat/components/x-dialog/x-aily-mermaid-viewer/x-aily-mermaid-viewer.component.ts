    ...
    <div class="ac-mermaid">
      @if (!diagramReady && !renderError) {
        <div class="ac-mermaid-pending">
          ...
          <span>{{
            streamStatus === 'loading' ? '正在生成图表…' : '正在渲染图表…'
          }}</span>
        </div>
      }
      <div
        class="ac-mermaid-svg"
        #diagramContainer
        [style.display]="diagramReady ? '' : 'none'"
      ></div>
      @if (renderError) {
        <div class="ac-mermaid-err">{{ renderError }}</div>
      }
    </div>
  `,
  styles: [
    `
      ...
      .ac-mermaid-svg ::ng-deep svg {
        max-width: 100%;
        height: auto;
        background: transparent !important;
      }
      .ac-mermaid-svg ::ng-deep .label-container {
        fill: var(--surface-muted) !important;
      }
      .ac-mermaid-err {
        padding: 16px;
        display: flex;
...
export class XAilyMermaidViewerComponent implements OnChanges, OnDestroy {
  ...
  private async doMermaid(src: string): Promise<void> {
    ...
    try {
      ...
      off.style.cssText =
        'position:fixed;left:-9999px;top:-9999px;visibility:hidden;z-index:-1';
  ...
  private withScrollGuard(fn: () => void): void {
    ...
    while (el) {
      if (el.scrollTop !== 0 || el.scrollLeft !== 0)
        snap.push([el, el.scrollTop, el.scrollLeft]);
      el = el.parentElement;
    }
    fn();
    for (const [e, t, l] of snap) {
      e.scrollTop = t;
      e.scrollLeft = l;
    }
  }
