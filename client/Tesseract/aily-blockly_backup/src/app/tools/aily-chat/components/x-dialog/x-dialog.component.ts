...
export class XDialogComponent implements OnChanges {
  ...
  private fixContent(content: string): string {
    ...
    const ailyTypes = ['aily-blockly', 'aily-board', 'aily-library', 'aily-state',
      ...
      'aily-workflow-blueprint', 'aily-component-recommendation', 'aily-config-guide'];
    ...
    return content
      ...
      .replace(/\s*```(aily-(?:board|library|state|button|task-action|think|context|workflow-blueprint|component-recommendation|config-guide))/g, '\n```$1\n');
  }
