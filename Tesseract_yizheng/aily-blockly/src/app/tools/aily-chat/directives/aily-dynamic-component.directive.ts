...
export class AilyDynamicComponentDirective implements OnInit, OnDestroy {
    ...
    this.observer = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
          ...
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (element.classList?.contains('aily-code-block-placeholder')) {
                const placeholderKey = this.generatePlaceholderKey(
              ...
              // 检查元素内部是否包含占位符
              const innerPlaceholders = element.querySelectorAll?.(
                '.aily-code-block-placeholder',
              );
        ...
        if (
          ...
          if (
            target.classList?.contains('aily-code-block-placeholder') &&
  ...
  private scanAndReplaceComponents(): void {
    const placeholders = this.elementRef.nativeElement.querySelectorAll(
      '.aily-code-block-placeholder',
    );
  ...
  private replacePlaceholderWithComponent(placeholder: HTMLElement): void {
    ...
    const existingComponent = this.findExistingComponent(
      ailyType,
      componentId,
      componentData,
    );
    ...
    if (
      ...
      typeof componentRef.instance.setData === 'function'
    ) {
  ...
  private showErrorPlaceholder(placeholder: HTMLElement, error: any): void {
    ...
    errorContainer.style.cssText = `
      ...
      padding: 12px;
      margin: 8px 0;
      background-color: color-mix(in srgb, var(--sem-danger) 10%, var(--surface-panel));
  ...
  private findExistingComponent(
    ...
    componentData: any,
  ): { container: HTMLElement; componentRef: ComponentRef<any> } | null {
   ...
   */
  private findExistingStateComponent(
    stateId: string,
  ): { container: HTMLElement; componentRef: ComponentRef<any> } | null {
   ...
   * 从占位符更新现有组件（用于属性变化时）
   */
  private updateExistingComponentFromPlaceholder(
    ...
    if (!componentContainer) {
      const allThinkContainers = this.elementRef.nativeElement.querySelectorAll(
        '.aily-think-container',
      );
      if (allThinkContainers.length > 0) {
        // 使用最后一个容器（应该对应最后一个段落）
        componentContainer = allThinkContainers[
          allThinkContainers.length - 1
        ] as HTMLElement;
      }
    }
    ...
    this.updateExistingComponent(
      { container: componentContainer, componentRef },
      componentData,
    );
  }
   ...
   * 更新现有组件的数据
   */
  private updateExistingComponent(
    existingComponent: {
