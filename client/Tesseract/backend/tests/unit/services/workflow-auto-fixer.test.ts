/**
 * [INPUT]: 依赖 WorkflowAutoFixer 与工作流校验错误协议
 * [OUTPUT]: 验证表达式修复、类型修复与 code 节点最小模板自动修复
 * [POS]: tests/unit/services 的自动修复守护测试
...
describe('WorkflowAutoFixer', () => {
  ...
  describe('Code Node Template Fixes', () => {
    it('should fill minimal executable code template for empty code nodes', async () => {
      const workflow = createMockWorkflow([
        createMockNode('node-1', 'n8n-nodes-base.code', {})
      ]);
      ...
      expect(result.fixes).toHaveLength(1);
      expect(result.fixes[0].type).toBe('code-node-template');
      expect(result.fixes[0].after).toMatchObject({
        language: 'javaScript',
        jsCode: 'return items;',
      });
      expect(result.operations).toHaveLength(1);
