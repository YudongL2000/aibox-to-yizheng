...
export class TesseractChatService {
  ...
  async chat(
    ...
    const result = await this.electronAPI.tesseract.chat({
      ...
      clarificationContext: options.clarificationContext || null,
    });
  ...
  async confirmWorkflow(sessionId: string, projectPath?: string) {
    this.requireSessionId(sessionId, '确认工作流');
    await this.startRuntime(projectPath);
    try {
      ...
      const fallbackResponse = {
        ...
        response: {
          type: 'error' as const,
          message: isTimeout
            ? '服务器响应超时，请稍后点击「确认构建」重试，或尝试简化需求描述。'
  ...
  private async syncDigitalTwinScene(
    ...
    // Guidance/workflow transitions often omit a scene entirely. In that case we
    // should preserve the current hardware/runtime scene instead of resetting the
    // digital twin back to the bare base chassis.
    if (scene == null) {
      return;
    }
