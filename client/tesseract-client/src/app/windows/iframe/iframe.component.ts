 ...
 * [OUTPUT]: 对外提供 IframeComponent 子窗口承载器，负责展示外部页面并按需建立数据桥，同时在数字孪生场景下重放当前 scene，在硬件组装模式下转发组件需求与桥接状态、接收完成通知并直接执行端侧 workflow upload/stop，在预览模式下将 MQTT 心跳设备映射为常驻的 mic/speaker/camera preview session 注入 Flutter 面板，并中继设备控制命令（麦克风/扬声器）到 Electron IPC，并使用更长加载窗容纳 Flutter Web DDC 启动。
 * [POS]: windows/iframe 的通用嵌入窗口，被侧边栏工具、连线图与数字孪生入口复用，是“页面承载”和“数据桥接”之间的薄层。
 * [PROTOCOL]: 变更时更新此头部，然后检查 AGENTS.md
 */

import { Component, ElementRef, Inject, OnDestroy, Optional, OnInit, ViewChild, NgZone } from '@angular/core';
...
export class IframeComponent implements OnInit, OnDestroy {
  ...
  private startDigitalTwinSceneListener(): void {
    if (!this.isDigitalTwinWindow || !window['electronAPI']?.digitalTwin?.onSceneUpdated) {
      this.stopDigitalTwinSceneListener();
      return;
    }
  ...
  private startDigitalTwinPreviewStateListener(): void {
    if (!this.isDigitalTwinWindow || !window['electronAPI']?.digitalTwin?.onPreviewStateUpdated) {
      this.stopDigitalTwinPreviewStateListener();
      return;
    }
  ...
  private startAssemblyBridgeRelay(): void {
    ...
    this.assemblyBridgeSubscription = combineLatest([
      ...
      // 合并两个数据源的组件列表（MQTT 运行时 + MiniClaw 桥接）
      const merged = new Map<string, { componentId: string; deviceId: string; displayName: string; status: string }>();
  ...
  private mapMqttDeviceToComponent(
  ...
  ): { componentId: string; deviceId: string; displayName: string; status: string } | null {
    ...
    return { componentId, deviceId: device.deviceName || `${componentId}-001`, displayName, status };
  }
    ...
    if (action === 'upload') {
      ...
      void electronAPI.tesseract.hardwareUpload({
        ...
        this.postAssemblyWorkflowStatus({
          action,
          success: true,
          message: result?.data?.statusText || result?.statusText || '工作流已下发到端侧',
          result,
        });
      }).catch((error: any) => {
        this.postAssemblyWorkflowStatus({
    ...
    if (action === 'stop') {
      void electronAPI.tesseract.hardwareStop({}).then((result: any) => {
        this.postAssemblyWorkflowStatus({
          action,
          success: true,
          message: result?.data?.statusText || result?.statusText || '端侧工作流已停止',
          result,
        });
      }).catch((error: any) => {
        this.postAssemblyWorkflowStatus({
  ...
  private setIframeUrl(url: string): void {
    ...
    this.isDigitalTwinWindow = url.includes('entry=digital-twin');
  ...
  private async syncCurrentDigitalTwinScene(): Promise<void> {
    if (!this.isDigitalTwinWindow || !window['electronAPI']?.digitalTwin?.getScene) {
      return;
    }

    try {
      const scene = await window['electronAPI'].digitalTwin.getScene();
      this.lastDigitalTwinEnvelopeSummary = this.summarizeDigitalTwinEnvelope(scene);
  ...
  private async syncCurrentDigitalTwinPreviewState(): Promise<void> {
    if (!this.isDigitalTwinWindow || !window['electronAPI']?.digitalTwin?.getPreviewState) {
      return;
    }

    try {
      const previewState = await window['electronAPI'].digitalTwin.getPreviewState();
      this.lastDigitalTwinPreviewEnvelopeSummary = this.summarizeDigitalTwinPreviewEnvelope(previewState);
  ...
  private scheduleDigitalTwinReplayCheck(): void {
    ...
    this.digitalTwinReplayTimer = setTimeout(() => {
      ...
      if (!this.digitalTwinChildReady && bootstrapAgeMs < DIGITAL_TWIN_BOOTSTRAP_GRACE_MS) {
        void this.syncCurrentDigitalTwinScene();
        return;
      }
