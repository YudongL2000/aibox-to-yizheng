/**
 * [INPUT]: 依赖 MCP in-memory transport、TestableN8NMCPServer 与工具错误路径
 * [OUTPUT]: 验证 MCP 工具错误码、参数错误与恢复语义
 * [POS]: integration/mcp-protocol 的错误护栏测试，锁住 Refactor-4 工具面简化后的边界行为
...
describe('MCP Error Handling', () => {
  ...
  describe('Tool-Specific Errors', () => {
    describe('Node Discovery Errors', () => {
      ...
      it('should reject legacy get_node mode parameters with migration guidance', async () => {
        try {
          await client.callTool({ name: 'get_node', arguments: {
          ...
          expect(error).toBeDefined();
          expect(error.message).toContain('get_node no longer accepts legacy parameter "mode"');
          expect(error.message).toContain('Use get_node_docs');
        }
      });
    });
