 ...
 * [POS]: tools/aily-chat/services 的对话模式桥接入口层，只产出标准事件/relay 结果，不做业务分支判断。
...
const DEFAULT_DIALOGUE_WS_URL = 'ws://192.168.1.150:18789/';
...
export class DialogueHardwareBridgeService implements OnDestroy {
  ...
  private runtimeConfigPromise: Promise<DialogueBridgeRuntimeConfig> | null = null;
  ...
  private readonly pendingRelayRequests = new Map<string, PendingRelayRequest>();
  ...
  dispatchPhysicalCue(cue: DialoguePhysicalCue | null, sessionId?: string): boolean {
  ...
  async relayChatMessage(
    ...
    return new Promise<DialogueRelayResult>((resolve, reject) => {
      const pending = this.createPendingRelayRequest(chatId, idleMs, resolve, reject);
      ...
      try {
        ...
        this.rejectRelayRequest(chatId, error?.message || 'MimicLaw 消息发送失败');
      }
    });
  }
  ...
  private applyEvent(event: DialogueHardwareEvent): void {
    ...
    this.stateSubject.next({
      ...
      lastError: event.eventType === 'device_error'
        ? String(event.raw?.['content'] || event.raw?.['message'] || '设备异常')
        : null,
    });
  }
  ...
  private reduceComponents(
    ...
    if (event.eventType === 'snapshot') {
      return (event.connectedComponents || []).filter((item) => item.status !== 'removed');
    }
  ...
  private normalizeEvent(payload: unknown): DialogueHardwareEvent | null {
    ...
    const connectedComponents = this.readConnectedComponents(map, text, component);
    const eventType = this.inferEventType(map, text, component);

    // 若心跳携带设备列表则升级为 snapshot，触发 reduceComponents 替换组件列表
    const effectiveEventType = (eventType === 'heartbeat' && connectedComponents.length > 0)
      ? 'snapshot'
      : eventType;

    if (!component && connectedComponents.length === 0 && effectiveEventType === 'heartbeat') {
      return {
        source: 'miniclaw_ws',
  ...
  private readComponentFromSource(
    ...
    const notes = this.normalizeMap(source['notes'] || source['meta'] || source['metadata']);
    const componentId = this.pickString(source, [
      'componentId',
      'component_id',
      'deviceType',
      'device_type',
    ...
    return {
      componentId,
      deviceId: this.pickString(source, ['deviceId', 'device_id']) || `${componentId}-001`,
      modelId:
      ...
      portId: this.pickString(source, ['portId', 'port_id']) || this.inferPortId(text) || 'port_2',
      status,
    };
  }
  ...
  private readConnectedComponents(
    ...
    const componentArrays = [
      this.pickComponentArray(map, ['connectedComponents', 'connected_components', 'components', 'device_info', 'deviceInfo']),
      this.pickComponentArray(this.normalizeMap(map?.['hardware']), ['connectedComponents', 'connected_components', 'components', 'device_info', 'deviceInfo']),
      this.pickComponentArray(this.normalizeMap(map?.['snapshot']), ['connectedComponents', 'connected_components', 'components', 'device_info', 'deviceInfo']),
    ];
  ...
  private inferDeviceIdComponent(deviceId: string): DialogueHardwareComponent | null {
  ...
  private extractTextComponents(text: string): DialogueHardwareComponent[] {
    ...
    while ((match = linePattern.exec(normalizedText)) !== null) {
      const typeToken = String(match[1] || '').trim().toLowerCase();
      const statusToken = String(match[2] || '').trim().toLowerCase();
      const deviceId = String(match[3] || '').trim().toLowerCase();
      ...
      const portId = this.extractPortFromSegment(tail) || this.inferPortId(tail) || 'port_2';
  ...
  private extractDeviceIds(
    ...
    const rawDevices = map?.['devices'] ?? map?.['deviceIds'] ?? map?.['device_ids'];
    ...
    const devicesMatch = String(text || '').match(/(?:^|[\s|])devices\s*=\s*([^|\n\r]+)/i);
    if (!devicesMatch?.[1]) {
      return [];
    }
  ...
  private inferEventType(
    ...
    if (/insert|attach|connect|plug|online|插入|已连接|在线/.test(normalizedText)) {
  ...
  private normalizeEventType(value: string): DialogueHardwareEventType {
    ...
    if (value.includes('error') || value.includes('fail')) return 'device_error';
    if (value.includes('insert') || value.includes('attach') || value.includes('connect')) {
      return 'device_inserted';
    }
  ...
  private readStatus(
    ...
    const explicit = String(map?.['status'] || '').trim().toLowerCase();
    if (explicit) {
      ...
      if (explicit.includes('error') || explicit.includes('fail')) return 'error';
      if (explicit.includes('valid')) return 'validating';
      if (explicit.includes('online') || explicit.includes('connect')) return 'connected';
      return 'connected';
    }

    const normalizedText = text.toLowerCase();
    if (/remove|detach|disconnect|拔出/.test(normalizedText)) return 'removed';
    if (/ready|validated|success|准备好|校验成功/.test(normalizedText)) return 'ready';
  ...
  private inferComponentId(text: string): string {
    ...
    if (/\bhand\d*\b|mechanical[_ -]?hand|claw|夹爪|机械手/.test(normalizedText)) return 'mechanical_hand';
    if (/\bspk\d*\b|speaker|喇叭|音箱/.test(normalizedText)) return 'speaker';
    if (/\bmic\d*\b|mic|microphone|麦克风/.test(normalizedText)) return 'microphone';
    if (/\bcar\d*\b|\bwheel\d*\b|chassis|底盘|小车/.test(normalizedText)) return 'chassis';
    if (/\bscreen(?:[_ -]?\d+)?\b|\bdisplay(?:[_ -]?\d+)?\b|屏幕|显示屏/.test(normalizedText)) return 'screen';
    return '';
  }
  ...
  private async loadRuntimeConfig(): Promise<DialogueBridgeRuntimeConfig> {
    const configuredWsUrl =
      await this.readBridgeSetting(DIALOGUE_WS_URL_ENV_KEY, DIALOGUE_WS_URL_DEBUG_KEY);
    const configuredTimeout =
      await this.readBridgeSetting(DIALOGUE_RELAY_TIMEOUT_ENV_KEY, DIALOGUE_RELAY_TIMEOUT_DEBUG_KEY);
    const configuredIdle =
      await this.readBridgeSetting(DIALOGUE_RELAY_IDLE_ENV_KEY, DIALOGUE_RELAY_IDLE_DEBUG_KEY);

    return {
      wsUrl: this.normalizeWsUrl(configuredWsUrl, DEFAULT_DIALOGUE_WS_URL),
      relayTimeoutMs: this.normalizePositiveInt(configuredTimeout, DEFAULT_RELAY_TIMEOUT_MS),
      relayIdleMs: this.normalizePositiveInt(configuredIdle, DEFAULT_RELAY_IDLE_MS),
    };
  }

  private async readBridgeSetting(envKey: string, debugKey: string): Promise<string> {
  ...
  private readRendererDebugValue(key: string): string {
    try {
      const queryValue = new URLSearchParams(globalThis.location?.search || '').get(key);
  ...
  private resolveRelayRequest(chatId: string): void {
    ...
    pending.resolve({
      chatId,
      content: pending.messages.map((item) => item.content).join('\n\n').trim(),
  ...
  private resolveRelayChatId(requestedChatId?: string | null): string {
    ...
    const digest = this.hashToken(`${requestedChatId || ''}:${timestamp}:${Math.random()}`);
    return `${RELAY_CHAT_ID_PREFIX}-${timestamp}-${digest}`
      .slice(0, MAX_SAFE_RELAY_CHAT_ID_LENGTH);
  }
