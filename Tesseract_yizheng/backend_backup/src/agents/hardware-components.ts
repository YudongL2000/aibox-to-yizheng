/**
 * [INPUT]: 依赖 types.ts 的 NodeCategory/HardwareCapabilityEndpoint，依赖 hardware-capability-ids.ts 的规范组件/能力 id 常量、hardware-endpoint-defaults.ts 的 endpoint 真相源，以及 utils/code-node-parameters.ts 的 code 节点模板
 * [OUTPUT]: 对外提供 SCENE_NODE_TYPE_VERSIONS、SCENE_NOTES_FIELDS、SCENE_COMPONENT_DEFINITIONS 与 HARDWARE_COMPONENTS
 * [POS]: agents 的组件能力真相层；场景定义与默认 endpoint 都只允许从这里和专用 constants 派生
...
import {
  buildHardwareCapabilityUrl,
  ROBOT_CAMERA_SNAPSHOT_URL,
  ROBOT_MIC_SNAPSHOT_URL,
...
function buildCapabilityEndpoint(
  ...
  return {
    url: buildHardwareCapabilityUrl(component.componentId, capability.id),
    method: 'POST',
  };
}
