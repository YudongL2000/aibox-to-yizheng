...
const { buildDevServerUrl, getDevServerBaseUrl, resolveRendererIndexPath } = require("./renderer-path");
...
const { registerTesseractHandlers, stopAllRuntimes } = require("./tesseract-ipc");

let mainWindow;
let userConf;
let isRendererReady = false;
let isQuittingRuntimes = false;
...
function updateMainWindowWithPendingData() {
  ...
  if (targetUrl) {
    if (serve) {
      mainWindow.loadURL(buildDevServerUrl(targetUrl));
    } else {
      mainWindow.loadFile(resolveRendererIndexPath(__dirname), { hash: targetUrl });
    }
  }
}
...
function createWindow() {
  ...
  mainWindow.webContents.on('console-message', (details) => {
    ...
    const label = labelMap[details.level] || 'info';
    const source = details.sourceId ? path.basename(details.sourceId) : 'renderer';
    console[label](`[renderer:${source}:${details.lineNumber}] ${details.message}`);
  });
  ...
  if (targetUrl) {
    ...
    if (serve) {
      mainWindow.loadURL(getDevServerBaseUrl());
    } else {
      mainWindow.loadFile(resolveRendererIndexPath(__dirname));
    }
  }
  ...
  const windowApi = registerWindowHandlers(mainWindow);
  ...
  registerTesseractHandlers(mainWindow);

  // 数字孪生预加载 — 主窗口渲染完成后后台加载，减少首次打开等待
  mainWindow.webContents.once('did-finish-load', () => {
    const desktopTwinMode = process.env.TESSERACT_DESKTOP_TWIN_MODE || 'embedded-panel';
    if (desktopTwinMode === 'legacy-window' && windowApi?.preloadDigitalTwin) {
      windowApi.preloadDigitalTwin();
    }
  });
...
app.on("before-quit", (event) => {
  if (isQuittingRuntimes) {
    return;
  }

  isQuittingRuntimes = true;
  event.preventDefault();
  Promise.resolve(stopAllRuntimes())
    .catch((error) => {
      console.warn("Failed to stop desktop runtimes before quit:", error);
    })
    .finally(() => {
      app.exit(0);
    });
...
app.on('open-file', (event, filePath) => {
  ...
  if (filePath.endsWith('.abi') && fs.existsSync(filePath)) {
    ...
    if (mainWindow && mainWindow.webContents) {
      ...
      if (serve) {
        mainWindow.loadURL(buildDevServerUrl(`#/${routePath}`));
      } else {
        mainWindow.loadFile(resolveRendererIndexPath(__dirname), { hash: `#/${routePath}` });
      }
    } else {
