/**
 * [INPUT]: 依赖 LogService/NzMessageService/UiService/ElectronService 与 renderer 结构化日志桥。
 * [OUTPUT]: 对外提供 LogComponent，负责日志面板展示、复制、导出与发送到 AI。
 * [POS]: tools/log 的 UI 容器，承接 renderer 内部日志列表交互。
...
export class LogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async copyLogItemToClipboard(item: any) {
    try {
      ...
      this.emitLog('warn', '[LogComponent] 复制到剪切板失败', err);
    }
  }
