/**
 * [INPUT]: 依赖 better-sqlite3 的原生构造器、数据库 schema 文件与 DatabaseAdapter 接口
 * [OUTPUT]: 对外提供 better-sqlite3 可用性探测、测试数据库工厂、事务辅助与测试适配器
 * [POS]: tests/integration/database 的共享基座，负责把“原生 SQLite 可用性”与具体测试用例解耦
...
let betterSqlite3Available: boolean | null = null;

function loadBetterSqlite3(): BetterSqlite3Module {
  if (betterSqlite3) return betterSqlite3;
  try {
    const loadedModule = require('better-sqlite3') as BetterSqlite3Module;
...
function probeBetterSqlite3(): BetterSqlite3Module {
  if (betterSqlite3Available === true && betterSqlite3) {
...
export function isBetterSqlite3Available(): boolean {
  if (betterSqlite3Available !== null) {
    return betterSqlite3Available;
  }
  try {
    probeBetterSqlite3();
    return true;
...
export function getBetterSqlite3Ctor(): BetterSqlite3Module | null {
  try {
    return probeBetterSqlite3();
  } catch {
    return null;
  }
}
...
export function isFts5Available(): boolean {
  if (!isBetterSqlite3Available()) return false;
  try {
    const DatabaseCtor = probeBetterSqlite3();
    const db = new DatabaseCtor(':memory:');
