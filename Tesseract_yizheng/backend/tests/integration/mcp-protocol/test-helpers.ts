...
export class TestableN8NMCPServer {
  ...
  constructor() {
    ...
    if (fs.existsSync(prodDbPath)) {
      this.testDbPath = prodDbPath;
      this.shouldCleanupDb = false;
    } else {
      const testDir = path.join(process.cwd(), '.test-dbs');
      if (!fs.existsSync(testDir)) {
        fs.mkdirSync(testDir, { recursive: true });
      }
      this.testDbPath = path.join(testDir, `n8n-mcp-test-${process.pid}-${instanceId}.db`);
      this.shouldCleanupDb = true;
    }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      if (this.shouldCleanupDb && !fs.existsSync(this.testDbPath)) {
        const prodDbPath = path.join(process.cwd(), 'data', 'nodes.db');
  ...
  async close(): Promise<void> {
    ...
    const performClose = async () => {
      ...
      // Close underlying MCP server resources (DB/cache)
      try {
    ...
    if (this.testDbPath && this.shouldCleanupDb) {
      try {
