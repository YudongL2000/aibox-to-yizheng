...
- 硬件组装页本地 overlay 如果消费的是 runtime/bridge 硬件状态，进入 `DigitalTwinModelItem.interfaceId` 前必须先把 raw `portId` 规范化成 canonical `port_*`；否则会和首页 backend-first scene 产生不同的挂载结果。
...
变更日志
- 2026-04-15: `home_workspace_page.dart` 的组装 overlay 合并逻辑开始规范化 raw `3-1.x` / HDMI 端口别名，修复组装态使用 bridge 原始 portId 时的挂载错位。
