...
describe('TesseractStudioComponent', () => {
  ...
  beforeEach(() => {
    ...
    tesseractProjectService = {
      ...
      peekWorkflowViewTarget: jasmine.createSpy('peekWorkflowViewTarget').and.returnValue(null),
    ...
    (window as any).electronAPI = {
      n8n: {
        ...
        getEditorUrl: jasmine.createSpy('getEditorUrl').and.resolveTo({
  ...
  it('does not call webview methods before the frame is attached to the DOM', async () => {
    const detachedWebview = {
      isConnected: false,
      loadURL: jasmine.createSpy('loadURL').and.callFake(() => {
        throw new Error('should not be called while detached');
      }),
    ...
    component.workspaceFrame = {
      nativeElement: detachedWebview,
    } as any;
    component.projectPath = '';
    component.requestedWorkflowId = 'wf-1';

    await component.loadWorkspace();

    expect((window as any).electronAPI.n8n.getEditorUrl).toHaveBeenCalledWith({
      projectPath: undefined,
      workflowId: 'wf-1',
    });
    expect(detachedWebview.loadURL).not.toHaveBeenCalled();
    const editorUrl = String(component.safeEditorUrl || '');
    expect(editorUrl).toContain('/workflow/wf-1');
    expect(editorUrl).toContain('embedded=electron');
  });
});
