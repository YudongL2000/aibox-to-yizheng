/**
 * [INPUT]: 依赖 MCP in-memory transport、TestableN8NMCPServer 与核心工具协议
 * [OUTPUT]: 验证 MCP 工具调用主路径，包括节点查询、节点拆分工具与校验链路
 * [POS]: integration/mcp-protocol 的主行为测试，锁住 Refactor-4 工具面拆分后的真实调用路径
...
describe('MCP Tool Invocation', () => {
  ...
  describe('Node Discovery Tools', () => {
    ...
    describe('split node tools', () => {
      it('should return readable documentation via get_node_docs', async () => {
        const response = await client.callTool({ name: 'get_node_docs', arguments: {
          nodeType: 'nodes-base.httpRequest'
        }});

        const result = JSON.parse(((response as any).content[0]).text);
        expect(result).toHaveProperty('nodeType', 'nodes-base.httpRequest');
        expect(result).toHaveProperty('documentation');
        expect(typeof result.documentation).toBe('string');
      });

      it('should search node properties via dedicated tool', async () => {
        const response = await client.callTool({ name: 'search_node_properties', arguments: {
          nodeType: 'nodes-base.httpRequest',
          query: 'auth'
        }});
      ...
      it('should return version history and upgrade details via get_node_versions', async () => {
        const versionedNodeType = 'nodes-base.executeWorkflow';
        const historyResponse = await client.callTool({ name: 'get_node_versions', arguments: {
          nodeType: versionedNodeType
        }});
        const history = JSON.parse(((historyResponse as any).content[0]).text);

        expect(history).toHaveProperty('nodeType', versionedNodeType);
        expect(history).toHaveProperty('versions');

        const upgradeResponse = await client.callTool({ name: 'get_node_versions', arguments: {
          nodeType: versionedNodeType,
          since: '1.0'
        }});
        const upgrade = JSON.parse(((upgradeResponse as any).content[0]).text);

        expect(upgrade).toHaveProperty('history');
        expect(upgrade).toHaveProperty('targetVersion');
      });
    });
  });
