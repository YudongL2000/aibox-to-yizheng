...
export class N8nApiClient {
  ...
  private static readonly CORE_SETTINGS_KEYS = new Set([
  ...
  private static readonly MINIMAL_SAFE_SETTINGS: Record<string, unknown> = {
  ...
  async createWorkflow(workflow: Partial<Workflow>): Promise<Workflow> {
    try {
      ...
      // Apply settings compatibility filtering for workflow creation as well.
      // This prevents create failures on older n8n versions when new properties are present.
      const versionInfo = await this.getVersion();
      if (versionInfo) {
        ...
        logger.warn('Could not determine n8n version, using conservative settings properties for create');
        cleanedWorkflow.settings = this.buildConservativeSettings(
          cleanedWorkflow.settings as Record<string, unknown> | undefined
        );
      }
      ...
      try {
        ...
        // Some instances still reject filtered settings. Retry once with minimal-safe core settings.
        if (!this.isSettingsAdditionalPropertiesError(error)) {
          throw error;
        }

        logger.warn('Create workflow rejected settings payload, retrying with minimal compatible settings');

        const retryWorkflow = {
          ...cleanedWorkflow,
          settings: { ...N8nApiClient.MINIMAL_SAFE_SETTINGS },
        };
        const retryResponse = await this.client.post('/workflows', retryWorkflow);
  ...
  private buildConservativeSettings(
    settings: Record<string, unknown> | undefined
  ): Record<string, unknown> {
    if (!settings || typeof settings !== 'object') {
      return { ...N8nApiClient.MINIMAL_SAFE_SETTINGS };
    }

    const filtered: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(settings)) {
      if (N8nApiClient.CORE_SETTINGS_KEYS.has(key)) {
        filtered[key] = value;
      }
    }

    return Object.keys(filtered).length > 0 ? filtered : { ...N8nApiClient.MINIMAL_SAFE_SETTINGS };
  }

  private isSettingsAdditionalPropertiesError(error: unknown): boolean {
    const n8nError = handleN8nApiError(error);
    if (n8nError.code !== 'VALIDATION_ERROR') {
      return false;
    }

    const text = `${n8nError.message} ${JSON.stringify(n8nError.details ?? '')}`.toLowerCase();
    return text.includes('settings') && text.includes('additional properties');
  }
