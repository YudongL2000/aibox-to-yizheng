...
tesseract-runtime.js: Tesseract Agent runtime 管理器，优先复用已启动 backend，再按需拉起本地进程并代理 agent/workflow/skills/hardware HTTP；支持 `TESSERACT_BACKEND_MODE=external`，把桌面端切到“只复用外部 backend、不自启托管 backend”的调试模式，并在被动状态查询时把“backend 未就绪”降级成结构化 disconnected 状态而不是致命 IPC 异常。
...
- external backend 模式下，`tesseract-hardware-status` 这类被动状态 IPC 在 backend 缺席时必须返回结构化 `disconnected` 快照；只有真正需要执行后端动作的调用才应该抛错。
...
- Electron 注入 backend env 时不得用空字符串覆盖 `backend/.env` 的 `N8N_API_URL/N8N_API_KEY`；拿不到嵌入态 API 凭据时应让 backend 自己从 cwd 下的 `.env` 解析。
...
- `tesseract-create-workflow` 必须先 `await n8nRuntime.start()` 再转发给 backend；否则共享 `api-access.json` 还没生成时，backend 会错误回退到过期的 `.env` API key 并把 n8n `401 unauthorized` 透回 renderer。
- 比较 embedded n8n URL 时必须把 `localhost`/`127.0.0.1` 视为同一 loopback 实例；host 写法不同不能成为忽略共享 `api-access.json` 的理由。
...
变更日志
- 2026-04-12: tesseract-runtime.js 改为“仅在有值时注入 N8N_API_*”，避免 Electron 用空 env 屏蔽 backend/.env，导致 workflow create 在日志中假报未配置。
- 2026-04-12: `tesseract-create-workflow` 现在先确保 embedded n8n 启动并写出 `api-access.json`，修复 workflow create 首次请求命中过期 `.env` API key 后直接返回 `unauthorized` 的问题。
- 2026-04-12: embedded n8n API URL 比较新增 loopback host 归一化，避免 `.env` 写 `localhost`、共享凭据写 `127.0.0.1` 时错误回退到旧 key。
