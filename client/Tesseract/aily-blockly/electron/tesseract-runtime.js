 ...
 * [OUTPUT]: 对外提供 TesseractRuntime，负责复用已启动 Agent 服务或按需拉起本地 backend，并统一管理 IPC->HTTP 请求超时、dialogue-mode 调用、skills 库访问与结构化运行日志。
...
class TesseractRuntime {
  ...
  async start(options = {}) {
    ...
    if (this.backendMode === "external") {
      this.external = true;
      this.lastError = `External backend mode is enabled, but no backend is listening on ${this.host}:${candidatePorts[0] || configuredPort || DEFAULT_PORT}`;
      this.appendLog(
        `[tesseract-runtime] External backend mode requires a running backend on ${this.host}:${candidatePorts[0] || configuredPort || DEFAULT_PORT}`,
        "warn",
        "runtime"
      );
    ...
    const env = {
      ...
      N8N_API_URL: n8nApiAccess.baseUrl || "",
      N8N_API_KEY: n8nApiAccess.apiKey || "",
    };

    this.logs = [];
    this.lastError = "";
    this.appendLog(`[tesseract-runtime] Using n8n API config from ${n8nApiAccess.source}`);
    this.process = spawn(process.execPath, [runtime.entry], {
      cwd: runtime.root,
      env,
