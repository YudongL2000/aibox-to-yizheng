 ...
 * [OUTPUT]: 对外提供 SplashPage 启动判流页，负责缓存初始化、登录判定与本地嵌入数字孪生直达。
...
class _LaunchTarget {
  ...
  const _LaunchTarget._(this.kind, {this.promptText});

  const _LaunchTarget.home() : this._(_LaunchTargetKind.home);

  const _LaunchTarget.digitalTwin({String? promptText})
      : this._(_LaunchTargetKind.digitalTwin, promptText: promptText);
...
class _SplashPageState extends State<SplashPage> {
  ...
  _LaunchTarget _resolveLaunchTarget() {
    ...
    if (entry == 'digital-twin' || entry == 'workspace') {
      return _LaunchTarget.digitalTwin(promptText: promptText);
    }
