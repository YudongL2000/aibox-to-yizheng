...
- 2026-04-12: 新增 hardware-endpoint-defaults.ts，把 robot.local / ai.local / hardware-api fallback 与 n8n env-backed expression 收口为单一真相源；删除未接入主链路的 prompts.ts。
...
- 2026-04-12: orchestrator.ts 的 confirm 链路不再把“summary 已允许继续但显式 capability 为空”的场景打回 guidance，而是继续走 WorkflowArchitect/ComponentComposer 生成最小工作流，修复确认后循环卡死。
