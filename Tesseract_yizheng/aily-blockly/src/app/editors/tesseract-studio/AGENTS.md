...
成员清单
tesseract-studio.component.ts: Tesseract 工作区页面入口，负责读取路由可选 `path/workflowId/surface`、消费项目级 workflow sync target，并把 workflow 画布与 digital twin 组织成由 shell header 驱动的主视图；当项目已有未下发 workflow 时，也在这里提供“进入组装下发”入口并恢复数字孪生组装闭环。
tesseract-studio.component.spec.ts: 工作区同步与主视图 tab 回归测试，锁住全局活动工作区 `workflowId` 聚焦、延迟结果防串台、workspace tab 记忆行为，以及 workflow 页直接恢复硬件组装入口。
...
- workflow 页的“进入组装下发”只能复用项目快照里的 `sessionId/assemblyResume/hardwareDispatch` 真相；不得在页面里再手搓第二套组装状态缓存。

变更日志
- 2026-04-15: workflow 页新增“进入组装下发”入口；若本地快照可恢复 hot_plugging，则直接切到数字孪生组装态，否则按 session 恢复 config-state 并在可下发时直达硬件下发。
