...
describe('N8nApiClient', () => {
  ...
  describe('createWorkflow', () => {
    ...
    it('should retry with minimal settings when n8n rejects additional settings properties', async () => {
      const workflow = {
        name: 'Test Workflow',
        nodes: [],
        connections: {},
      ...
      const validationError = createAxiosError({
        ...
        response: {
          status: 400,
          data: { message: 'request/body/settings must NOT have additional properties' },
        },
      });
      const createdWorkflow = { ...workflow, id: 'wf-1' };

      mockAxiosInstance.post
        .mockRejectedValueOnce(validationError)
        .mockResolvedValueOnce({ data: createdWorkflow });

      const result = await client.createWorkflow(workflow);

      expect(mockAxiosInstance.post).toHaveBeenCalledTimes(2);
      expect(mockAxiosInstance.post).toHaveBeenNthCalledWith(2, '/workflows', expect.objectContaining({
        settings: {
