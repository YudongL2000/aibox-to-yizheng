...
const describeN8n = isN8nApiAccessibleSync() ? describe : describe.skip;

describeN8n('Integration: handleAutofixWorkflow', () => {
      ...
      const response = await handleAutofixWorkflow(
        {
          ...
          fixTypes: ['expression-format', 'typeversion-correction', 'error-output-config', 'webhook-missing-path']
        },
        repository,
        mcpContext
      );
