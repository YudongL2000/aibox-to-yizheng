/**
 * [INPUT]: 依赖 WorkflowValidator 与可控的 repository/config-validator mock
 * [OUTPUT]: 验证工作流校验器在结构、连接与 fix 元数据透传上的行为
 * [POS]: tests/unit/services 的 workflow-validator mock 驱动测试
...
describe('WorkflowValidator - Simple Unit Tests', () => {
  ...
  describe('Basic validation scenarios', () => {
    it('should fail validation for a single-node webhook workflow', async () => {
      // Arrange
      const nodeData = {
      ...
      expect(
        result.errors.some((e) =>
          e.message.includes('Single-node workflows are invalid for Tesseract')
        )
      ).toBe(true);
    });
    ...
    it('should normalize and validate nodes-base prefix to find the node', async () => {
      ...
      const mockRepository = {
        getNode: vi.fn((type: string) => {
          ...
          if (type === 'nodes-base.set') {
            return nodeData['nodes-base.set'];
          }
          return null;
        }),
        findSimilarNodes: vi.fn().mockReturnValue([])
      ...
      // Assert - The nodes should be found through normalization
      expect(result.valid).toBe(true);
    ...
    it('should preserve fix metadata from node validation errors', async () => {
      const nodeData = {
        'nodes-base.manualTrigger': {
          type: 'nodes-base.manualTrigger',
          displayName: 'Manual Trigger',
      ...
      const mockRepository = {
        getNode: vi.fn((type: string) => {
          if (type === 'nodes-base.code') {
            return nodeData['nodes-base.code'];
          }
          return null;
        }),
        findSimilarNodes: vi.fn().mockReturnValue([])
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.find((error) => error.message === 'Code cannot be empty')).toMatchObject({
        message: 'Code cannot be empty',
        code: 'CODE_NODE_EMPTY',
        fix: 'Add your code logic',
        details: { field: 'jsCode' },
