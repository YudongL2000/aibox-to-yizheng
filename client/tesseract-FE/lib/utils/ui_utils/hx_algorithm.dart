...
class HxAlgorithm extends Algorithm {
  ...
  Size run(Graph? graph, double shiftX, double shiftY) {
    ...
    firstWalk1(graph, firstNode, 0, 0);
  ...
  void firstWalk1(Graph graph, Node node, int depth, int number) {
    ...
    if (isLeaf(graph, node)) {
      ...
      final leftMost = getLeftMostChild(graph, node); //最左边的崽节点
      final rightMost = getRightMostChild(graph, node); //最右边的崽节点
      var defaultAncestor = leftMost; //默认祖先

      Node? next = leftMost;
      var i = 1;
      while (next != null) {
        firstWalk1(graph, next, depth + 1, i++); //递归
        defaultAncestor = apportion(graph, next, defaultAncestor); //默认祖先

        next = getRightSibling(graph, next); //右边的兄弟
      }
      ...
      var midPoint = 0.5 *
          ((getPrelim(leftMost) +
              ...
              (vertical ? node.width : node.height)); //中间位置
      // if (successorsOf(node).isNotEmpty) {
      //   //有儿子节点
      //   //最左边的子孙节点
      //   Node? nextLeft = _getMostLsftNode(graph, node);
      //   nodeData.prelim = getNodeData(nextLeft)!.prelim; //跟最左边儿子对齐
      // } else {
        if (hasLeftSibling(graph, node)) {
          if (successorsOf(node).isNotEmpty&&successorsOf(node).length>1) {
            //有儿子节点
            //最左边的子孙节点
            Node? nextLeft = _getMostLsftNode(graph, node);
            nodeData.prelim = getNodeData(nextLeft)!.prelim; //跟最左边儿子对齐
  ...
  Node _getMostLsftNode(Graph graph, Node node) {
    var nextLeft = this.nextLeft(graph, node);
    if (nextLeft == null) {
      return node;
    } else {
      return _getMostLsftNode(graph,nextLeft);
    }
  }
  ...
  void executeShifts(Graph graph, Node node) {
    ...
    var w = getRightMostChild(graph, node); //最右边的儿子
    while (w != null) {
      ...
      w = getLeftSibling(graph, w); //左边的兄弟
    }
  }
  ...
  Node apportion(Graph graph, Node node, Node defaultAncestor) {
    ...
    if (hasLeftSibling(graph, node)) {
      //左边有兄弟
      var leftSibling = getLeftSibling(graph, node); //获取到左边兄弟
      Node? vop = node;
      Node? vom = getLeftMostChild(
          graph, graph.predecessorsOf(node).first); //获取到第一个父亲的第一个儿子，即最左边的兄弟
      ...
      var nextRight = this.nextRight(graph, leftSibling); //左边兄弟的最后一个儿子
      ...
      for (nextLeft = this.nextLeft(graph, node);
        ...
        //一直循环到最左边的儿子
        vom = this.nextLeft(graph, vom); //左边兄弟的第一个儿子
        vop = this.nextRight(graph, vop); //最右边的儿子

        setAncestor(vop, node); //把当前设为最右边的儿子的祖先
        ...
        som += getModifier(vom);
        sop += getModifier(vop);
        nextRight = this.nextRight(graph, nextRight); //左边的最后一个儿子
      }
  ...
  HxAlgorithm(this.configuration, EdgeRenderer? renderer) {
