 ...
 * [OUTPUT]: 对外提供 FloatSiderComponent 浮动侧边栏，暴露项目设置、模型库、数字孪生等快捷入口，并以贴边 compact rail 呈现 hover label。
 * [POS]: components/float-sider 的交互入口层，挂载在 Tesseract/legacy 工作区右侧，负责把用户动作路由到工具或子窗口，同时保持与主壳层一致的悬浮语法。
...
export class FloatSiderComponent implements OnInit, OnDestroy {
  ...
  async openWorkflowTemplates() {
    this.uiService.openWorkbenchPanel('digital-twin', {
