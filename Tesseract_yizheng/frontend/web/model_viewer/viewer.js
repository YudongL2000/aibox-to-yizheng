 ...
 * [OUTPUT]: 对外提供多模型 3D viewer，支持只读展示与可编辑两种模式下的位姿同步、运行时缩放、灯光调节、滚轮缩放安全边界、只读缩略导航平移、shell 对齐的默认背景色与 viewerReady/transforms 的诊断日志。
 * [POS]: web/model_viewer 的核心渲染端，被 model_3d_viewer.dart 通过 iframe 协议驱动；当 `drag=false` 时退化成纯数字孪生展示层，主画布保留旋转/缩放，平移交给右下缩略导航图。
...
function normalizeHexColor(value, fallback = "#121316") {
...
function configureNavigationControls(controls, editable) {
  controls.enableDamping = true;
  controls.dampingFactor = 0.08;
  controls.screenSpacePanning = true;
  controls.enablePan = editable;
  controls.enableRotate = true;
  controls.enableZoom = true;
  controls.zoomSpeed = 0.9;
  controls.panSpeed = 1;
  controls.rotateSpeed = editable ? 0.9 : 0.78;
  ...
  // 只读数字孪生主画布只保留旋转/缩放，平移改由右下缩略导航图完成。
  controls.mouseButtons = {
    LEFT: THREE.MOUSE.ROTATE,
    MIDDLE: THREE.MOUSE.DOLLY,
...
function mapAssetsModelsPathToBundlePath(pathname) {
  const match = /^\/assets\/(?:assets\/)?models\/(.+)$/i.exec(pathname);
  if (match) {
    if (window.location.protocol === "file:") {
      return new URL(`../assets/assets/models/${match[1]}`, window.location.href).href;
    }
...
async function main() {
  ...
  configureNavigationControls(controls, editable);
  ...
  function refreshMiniMapBounds() {
    ...
    if (box.isEmpty()) {
      miniMapState.sceneBox.set(
        new THREE.Vector3(-0.5, 0, -0.5),
        new THREE.Vector3(0.5, 0, 0.5)
      );
    } else {
      miniMapState.sceneBox.copy(box);
    }

    const spanX = Math.max(
      miniMapState.sceneBox.max.x - miniMapState.sceneBox.min.x,
      0.5
    );
    const spanZ = Math.max(
      miniMapState.sceneBox.max.z - miniMapState.sceneBox.min.z,
      0.5
    );
    const span = Math.max(spanX, spanZ);
    const padding = Math.max(span * 0.22, 0.35);
    const half = span / 2 + padding;
    const centerX =
      (miniMapState.sceneBox.min.x + miniMapState.sceneBox.max.x) / 2;
    const centerZ =
      (miniMapState.sceneBox.min.z + miniMapState.sceneBox.max.z) / 2;
  ...
  function worldToMiniMapPoint(x, z) {
    const spanX = miniMapState.bounds.maxX - miniMapState.bounds.minX || 1;
    const spanZ = miniMapState.bounds.maxZ - miniMapState.bounds.minZ || 1;
    const u = (x - miniMapState.bounds.minX) / spanX;
    const v = (z - miniMapState.bounds.minZ) / spanZ;
    return {
      x: clamp(u, 0, 1) * miniMapCanvas.width,
      y: (1 - clamp(v, 0, 1)) * miniMapCanvas.height,
    };
  }

  function miniMapClientToWorldPoint(clientX, clientY) {
    ...
    const spanX = miniMapState.bounds.maxX - miniMapState.bounds.minX;
    const spanZ = miniMapState.bounds.maxZ - miniMapState.bounds.minZ;
    return {
      x: miniMapState.bounds.minX + u * spanX,
      z: miniMapState.bounds.minZ + v * spanZ,
    };
  }
  ...
  function drawMiniMap() {
    ...
    ctx.strokeStyle = "rgba(114, 121, 139, 0.18)";
    ctx.lineWidth = 1;
    ...
    const boxMin = worldToMiniMapPoint(
      miniMapState.sceneBox.min.x,
      miniMapState.sceneBox.min.z
    );
    const boxMax = worldToMiniMapPoint(
      miniMapState.sceneBox.max.x,
      miniMapState.sceneBox.max.z
    );
    const boxLeft = Math.min(boxMin.x, boxMax.x);
    const boxTop = Math.min(boxMin.y, boxMax.y);
    const boxWidth = Math.abs(boxMax.x - boxMin.x);
    const boxHeight = Math.abs(boxMax.y - boxMin.y);

    ctx.fillStyle = "rgba(115, 255, 180, 0.08)";
    ctx.strokeStyle = "rgba(115, 255, 180, 0.26)";
    ctx.lineWidth = 1.2;
    ...
    for (const wrapper of wrappers) {
      const point = worldToMiniMapPoint(wrapper.position.x, wrapper.position.z);
      ctx.beginPath();
      ctx.arc(point.x, point.y, 3.2, 0, Math.PI * 2);
      ctx.fill();
    }

    const targetPoint = worldToMiniMapPoint(controls.target.x, controls.target.z);
    const cameraPoint = worldToMiniMapPoint(camera.position.x, camera.position.z);

    ctx.strokeStyle = "rgba(129, 195, 255, 0.72)";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ...
    ctx.fillStyle = "rgba(129, 195, 255, 0.94)";
    ctx.beginPath();
    ctx.arc(cameraPoint.x, cameraPoint.y, 3.2, 0, Math.PI * 2);
    ctx.fill();

    ctx.strokeStyle = "rgba(255, 203, 107, 0.96)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(targetPoint.x - 6, targetPoint.y);
    ctx.lineTo(targetPoint.x + 6, targetPoint.y);
    ctx.moveTo(targetPoint.x, targetPoint.y - 6);
    ctx.lineTo(targetPoint.x, targetPoint.y + 6);
    ctx.stroke();

    ctx.fillStyle = miniMapState.dragging
      ? "rgba(255, 203, 107, 0.18)"
      : "rgba(255, 203, 107, 0.1)";
    ctx.beginPath();
    ctx.arc(targetPoint.x, targetPoint.y, miniMapState.dragging ? 10 : 8, 0, Math.PI * 2);
    ctx.fill();
  }

  function panCameraViaMiniMap(clientX, clientY) {
    if (!miniMapState.enabled) return;
    const worldPoint = miniMapClientToWorldPoint(clientX, clientY);
    if (!worldPoint) return;

    const deltaX = worldPoint.x - controls.target.x;
    const deltaZ = worldPoint.z - controls.target.z;
    camera.position.x += deltaX;
    camera.position.z += deltaZ;
    controls.target.x = worldPoint.x;
    controls.target.z = worldPoint.z;
    controls.update();
  ...
  function endMiniMapDrag(event) {
    ...
    if (
      ...
      event.pointerId !== miniMapState.pointerId
    ) {
      return;
    }
    if (
      miniMapState.pointerId !== null &&
      miniMapCanvas.hasPointerCapture?.(miniMapState.pointerId)
    ) {
      try {
        miniMapCanvas.releasePointerCapture(miniMapState.pointerId);
      } catch {
        // noop
      }
    }
  ...
  const miniMapState = {
    enabled: !editable && !!miniMapEl && !!miniMapCanvas && !!miniMapContext,
    dragging: false,
    pointerId: null,
    bounds: {
      minX: -1,
  ...
  if (miniMapState.enabled) {
    ...
    miniMapCanvas.addEventListener("pointerdown", (event) => {
      ...
      panCameraViaMiniMap(event.clientX, event.clientY);
    });

    miniMapCanvas.addEventListener("pointermove", (event) => {
      if (!miniMapState.dragging || event.pointerId !== miniMapState.pointerId) {
        return;
      }
      event.preventDefault();
      event.stopPropagation();
    ...
    miniMapCanvas.addEventListener("lostpointercapture", () => endMiniMapDrag());
  }
  ...
  try {
    ...
    if (hintEl) {
      hintEl.style.display = editable || miniMapState.enabled ? "block" : "none";
      if (editable) {
        hintEl.textContent = "点击组件选中 · 面板可切换平移/旋转模式";
      } else {
        hintEl.textContent = "左键拖拽旋转 · 滚轮缩放 · 右下缩略图平移";
      }
    }
    drawMiniMap();
