...
import type {
	...
	CanvasNodeTriggerRender,
	CanvasNodeSensorRender,
	CanvasNodeProcessorRender,
	CanvasNodeLogicRender,
	CanvasNodeActuatorRender,
	ExecutionOutputMap,
} from '../canvas.types';
...
export function useCanvasMapping({
	...
	function getNodeNotesObject(
		node: INodeUi,
	): { title?: unknown; subtitle?: unknown; function?: unknown } | undefined {
		const rawNotes = node.notes as unknown;

		if (!rawNotes) {
			return undefined;
		}

		// 1) 已经是对象的情况（后端直接返回 JSON）
		if (typeof rawNotes === 'object' && !Array.isArray(rawNotes)) {
			return rawNotes as { title?: unknown; subtitle?: unknown; function?: unknown };
		}

		// 2) 是字符串的情况，尝试按 JSON 解析
		if (typeof rawNotes === 'string') {
			try {
				const parsed = JSON.parse(rawNotes) as unknown;
				if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
					return parsed as { title?: unknown; subtitle?: unknown; function?: unknown };
				}
			} catch {
				// 不是 JSON 字符串，当作普通富文本备注，用不到 title/subtitle
				return undefined;
			}
		}
	...
	function getNodeNotesTitle(node: INodeUi): string | undefined {
		const notes = getNodeNotesObject(node);
		const title = notes?.title;

		return typeof title === 'string' && title.trim().length > 0 ? title : undefined;
	}

	function getNodeNotesSubtitle(node: INodeUi): string | undefined {
		const notes = getNodeNotesObject(node);
		const subtitle = notes?.subtitle;

		return typeof subtitle === 'string' && subtitle.trim().length > 0 ? subtitle : undefined;
	}

	function getNodeNotesFunction(node: INodeUi): string | undefined {
		const notes = getNodeNotesObject(node);
		const func = notes?.function;

		return typeof func === 'string' && func.trim().length > 0 ? func : undefined;
	}
	...
	function createTriggerRenderType(node: INodeUi): CanvasNodeTriggerRender {
		const nodeType = nodeTypeDescriptionByNodeId.value[node.id];
		const icon = getNodeIconSource(nodeType ?? node.type, node);

		// 优先从 notes 中读取标题和副标题
		const notesTitle = getNodeNotesTitle(node);
		const notesSubtitle = getNodeNotesSubtitle(node);
		const notesFunction = getNodeNotesFunction(node);

		// 获取节点副标题（使用已有的计算属性）
		const subtitle = nodeSubtitleById.value[node.id] ?? '';

		// 如果没有副标题，根据节点类型生成
		let finalSubtitle = notesSubtitle ?? notesFunction ?? subtitle;
		if (!finalSubtitle && nodeType) {
			const displayName = nodeType.displayName ?? nodeType.name.replace('n8n-nodes-base.', '');
			finalSubtitle = `FN: ${displayName}`;
			// 如果是 Schedule Trigger，添加时间间隔
			if (node.type === 'n8n-nodes-base.scheduleTrigger' && node.parameters.rule) {
				const rule = node.parameters.rule as { interval?: Array<{ field: string; expression: string }> };
				if (rule.interval && rule.interval.length > 0) {
					const interval = rule.interval[0];
					if (interval.field === 'seconds') {
						finalSubtitle += ` (${interval.expression}s)`;
					}
				}
			}
		}

		// 如果是 Webhook 节点，生成特定副标题
		if (node.type === 'n8n-nodes-base.webhook' && !finalSubtitle) {
			const method = (node.parameters.httpMethod as string | undefined) ?? 'POST';
			const path = (node.parameters.path as string | undefined) ?? '';
			finalSubtitle = `FN: Webhook (${method} /${path})`;
		}
		...
		return {
			...
			options: {
				icon,
				title: notesTitle ?? node.name,
				subtitle: finalSubtitle || 'TRIGGER',
				nodeType: node.type,
				displayName: nodeType?.displayName ?? node.name,
				httpMethod: node.parameters.httpMethod as string | undefined,
				path: node.parameters.path as string | undefined,
			},
		};
	}

	function createProcessorRenderType(node: INodeUi): CanvasNodeProcessorRender {
		const nodeType = nodeTypeDescriptionByNodeId.value[node.id];
		const icon = getNodeIconSource(nodeType ?? node.type, node);
		const notesTitle = getNodeNotesTitle(node);
		const notesSubtitle = getNodeNotesSubtitle(node);
	...
	function createActuatorRenderType(node: INodeUi): CanvasNodeActuatorRender {
		const nodeType = nodeTypeDescriptionByNodeId.value[node.id];
		const icon = getNodeIconSource(nodeType ?? node.type, node);
		const notesTitle = getNodeNotesTitle(node);
		const notesSubtitle = getNodeNotesSubtitle(node);
	...
	function createLogicRenderType(node: INodeUi): CanvasNodeLogicRender {
		const nodeType = nodeTypeDescriptionByNodeId.value[node.id];
		const icon = getNodeIconSource(nodeType ?? node.type, node);
		const notesTitle = getNodeNotesTitle(node);
		const notesSubtitle = getNodeNotesSubtitle(node);
	...
	function createSensorRenderType(node: INodeUi): CanvasNodeSensorRender {
		const nodeType = nodeTypeDescriptionByNodeId.value[node.id];
		const icon = getNodeIconSource(nodeType ?? node.type, node);
		const notesTitle = getNodeNotesTitle(node);
		const notesSubtitle = getNodeNotesSubtitle(node);
	...
	// 判断节点是否应该使用自定义 UI 的辅助函数
	function shouldUseCustomRenderType(node: INodeUi): CanvasNodeRenderType | null {
		const nodeName = node.name.toLowerCase();

		// 1. 触发器节点：所有 n8n 原始触发器分类都使用 Trigger UI
		// 包括 scheduleTrigger 等所有触发器类型
		if (isTriggerNodeById.value[node.id] || node.type === 'n8n-nodes-base.scheduleTrigger') {
			return CanvasNodeRenderType.Trigger;
		}

		// 2. 逻辑/判断器节点：优先根据节点类型判断
		if (node.type === 'n8n-nodes-base.if' ||
			node.type === 'n8n-nodes-base.splitInBatches' ||
			node.type === 'n8n-nodes-base.switch' ||
			node.type === 'n8n-nodes-base.filter') {
			return CanvasNodeRenderType.Logic;
		}

		// 3. 处理器节点：优先根据节点类型判断
		if (node.type === 'n8n-nodes-base.set') {
			return CanvasNodeRenderType.Processor;
		}

		// 4. 感知器节点：优先根据节点类型判断
		if (node.type === 'n8n-nodes-base.httpRequest') {
			return CanvasNodeRenderType.Sensor;
		}

		// 5. 执行器节点：优先根据节点类型判断
		if (node.type === 'n8n-nodes-base.code') {
			return CanvasNodeRenderType.Actuator;
		}

		// 6. 感知器节点：根据节点名称判断（备用）
		if (nodeName.includes('摄像头输入') || nodeName.includes('视觉环境快照')) {
			return CanvasNodeRenderType.Sensor;
		}
		if (nodeName.includes('麦克风输入') || nodeName.includes('语音音频拾取')) {
			return CanvasNodeRenderType.Sensor;
		}

		// 7. 处理器节点：根据图片映射判断（备用）
		const processorNodeMappings = [
			{ old: 'set camera payload', new: '图像规格标准化' },
			{ old: 'yolov8', new: 'ai 面部特征分析' },
			{ old: '识别-string(视觉)', new: '视觉情绪状态标记' },
			{ old: 'asr 语音识别', new: '语音转文字引擎' },
			{ old: '识别-string(asr文本)', new: '原始话语存证' },
			{ old: 'bert 情绪分类', new: '文本情感倾向分析' },
			{ old: '识别-string(文字)', new: '文本情绪状态标记' },
			{ old: '微笑响应', new: '共情行为编排' },
			{ old: '难过响应', new: '共情行为编排' },
			{ old: 'llm 文本回复', new: 'ai 共情逻辑大脑' },
			{ old: 'llm-string(用于tts)', new: '回复话术封装' },
			{ old: 'tts 文本', new: '拟人语音合成' },
			{ old: 'set audiourl', new: '音频资源定位' },
		];

		for (const mapping of processorNodeMappings) {
			if (nodeName.includes(mapping.old) || nodeName.includes(mapping.new)) {
				return CanvasNodeRenderType.Processor;
			}
		}

		// 8. 逻辑/判断器节点：根据节点名称判断（备用）
		if (nodeName.includes('判断') ||
			nodeName.includes('if') ||
			nodeName.includes('switch') ||
			nodeName.includes('filter') ||
			nodeName.includes('split')) {
			return CanvasNodeRenderType.Logic;
		}

		// 9. 执行器节点：根据图片映射判断（备用）
		const actuatorNodeMappings = [
			{ old: '屏幕emoji显示', new: '面部表情投射' },
			{ old: '机械臂动作执行', new: '肢体语言传动' },
			{ old: '喇叭播放音频', new: '语音合成播报' },
		];

		for (const mapping of actuatorNodeMappings) {
			if (nodeName.includes(mapping.old) || nodeName.includes(mapping.new)) {
				return CanvasNodeRenderType.Actuator;
			}
		}
	...
	const renderTypeByNodeId = computed(
		() =>
			nodes.value.reduce<Record<string, CanvasNodeData['render']>>((acc, node) => {
				...
				// 检查是否应该使用自定义 UI
				const customRenderType = shouldUseCustomRenderType(node);
				if (customRenderType) {
					switch (customRenderType) {
						case CanvasNodeRenderType.Trigger:
							acc[node.id] = createTriggerRenderType(node);
							break;
						case CanvasNodeRenderType.Sensor:
							acc[node.id] = createSensorRenderType(node);
							break;
						case CanvasNodeRenderType.Processor:
							acc[node.id] = createProcessorRenderType(node);
							break;
						case CanvasNodeRenderType.Logic:
							acc[node.id] = createLogicRenderType(node);
							break;
						case CanvasNodeRenderType.Actuator:
							acc[node.id] = createActuatorRenderType(node);
							break;
						default:
