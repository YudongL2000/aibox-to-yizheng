...
export class EnhancedConfigValidator extends ConfigValidator {
  ...
  private static validateResourceAndOperation(
    ...
    if (configWithDefaults.operation === undefined && configWithDefaults.resource !== undefined) {
      const defaultOperation = this.nodeRepository.getDefaultOperationForResource(
        normalizedNodeType,
        configWithDefaults.resource
      );
    ...
    if (config.resource !== undefined) {
      ...
      if (!resourceIsValid && config.resource !== '') {
        const samples = validResources.slice(0, 5).map(r => {
          const val = typeof r === 'string' ? r : r.value;
          return `"${val}"`;
        }).join(', ');
        const fix = samples
          ? `Valid resources: ${samples}${validResources.length > 5 ? '...' : ''}`
          : 'No resources found for this node.';

        result.errors.push({
          type: 'invalid_value',
          property: 'resource',
          message: `Invalid resource "${config.resource}" for node ${nodeType}.`,
          fix
        });
      }
    }
    ...
    if (config.operation !== undefined || configWithDefaults.operation !== undefined) {
      ...
      if (!operationIsValid && config.operation !== undefined && config.operation !== '') {
        const samples = validOperations.slice(0, 5).map(op => {
          const val = op.operation || op.value || op;
          return `"${val}"`;
        }).join(', ');
        const fix = samples
          ? `Valid operations${config.resource ? ` for resource "${config.resource}"` : ''}: ${samples}${validOperations.length > 5 ? '...' : ''}`
          : `No operations found${config.resource ? ` for resource "${config.resource}"` : ''}.`;

        let message = `Invalid operation "${config.operation}" for node ${nodeType}`;
        if (config.resource) {
          message += ` with resource "${config.resource}"`;
        }
        message += '.';
