/**
 * [INPUT]: 依赖 NodeRepository、EnhancedConfigValidator 与表达式校验器，聚合节点级错误/警告
 * [OUTPUT]: 对外提供 WorkflowValidator 与 ValidationIssue/WorkflowValidationResult
 * [POS]: services 的完整工作流校验器，负责保留节点级 fix 元数据并向 unified-validator 暴露可分流结果
...
import { isLangChainNode, isTriggerNode } from '../utils/node-type-utils';
...
export class WorkflowValidator {
  ...
  private validateWorkflowStructure(
    ...
    // Tesseract workflows must contain an executable chain, not just a trigger stub.
    if (workflow.nodes.length === 1) {
      const singleNode = workflow.nodes[0];
      const normalizedType = NodeTypeNormalizer.normalizeToFullForm(singleNode.type);
      const isLangchainNode = isLangChainNode(normalizedType);

      if (!isLangchainNode) {
        result.errors.push({
          type: 'error',
          message:
            'Single-node workflows are invalid for Tesseract. Add at least one more connected processing or execution node before returning workflow_ready.'
        });
      }
    }
      ...
      try {
        ...
        if (!nodeInfo) {
          result.errors.push({
            ...
            nodeId: node.id,
            nodeName: node.name,
            message: `Unknown node type: "${node.type}". Node types must include the package prefix (e.g., "n8n-nodes-base.webhook").`
          });
          continue;
        }
        ...
        // These nodes carry dynamic schemas that produce noisy false positives in strict validation.
        if (isLangChainNode(normalizedType)) {
          continue;
        }
        ...
        nodeValidation.errors.forEach((error: any) => {
          result.errors.push({
            ...
            details: typeof error === 'object' && error ? error.details : undefined,
            code: typeof error === 'object' && error && typeof error.code === 'string' ? error.code : undefined,
            fix: typeof error === 'object' && error ? error.fix : undefined,
          });
        });
        ...
        nodeValidation.warnings.forEach((warning: any) => {
          result.warnings.push({
            ...
            details: typeof warning === 'object' && warning ? warning.details : undefined,
            code: typeof warning === 'object' && warning && typeof warning.code === 'string' ? warning.code : undefined,
            fix: typeof warning === 'object' && warning ? warning.fix : undefined,
          });
        });
  ...
  private validateAIToolSource(
    ...
    // Treat langchain nodes and *Tool variants as valid ai_tool sources
    if (isLangChainNode(normalizedType) || normalizedType.endsWith('Tool')) {
      return;
    }
  ...
  private checkNodeErrorHandling(
    ...
    if (node.notes !== undefined) {
        const noteValue = node.notes;
        const isString = typeof noteValue === 'string';
        const isObject =
          typeof noteValue === 'object' && noteValue !== null && !Array.isArray(noteValue);
        if (!isString && !isObject) {
          result.errors.push({
            type: 'error',
            nodeId: node.id,
            nodeName: node.name,
            message: 'notes must be a string or object value'
          });
        } else if (isObject) {
          const record = noteValue as Record<string, unknown>;
          const stringKeys = [
            'title',
            'subtitle',
            'category',
          ...
          for (const key of stringKeys) {
            if (key in record && typeof record[key] !== 'string') {
              result.errors.push({
                ...
                nodeId: node.id,
                nodeName: node.name,
                message: `notes.${key} must be a string value`
              });
              break;
            }
          }

          const nullableStringKeys = ['topology', 'device_ID'];
            ...
            if (key in record) {
              ...
              if (value !== null && typeof value !== 'string') {
                result.errors.push({
                  ...
                  nodeId: node.id,
                  nodeName: node.name,
                  message: `notes.${key} must be a string or null value`
                });
                break;
              }
            }
          ...
          if ('sub' in record) {
            ...
            const isSubString = typeof subValue === 'string';
            const isSubObject =
              typeof subValue === 'object' && subValue !== null && !Array.isArray(subValue);
            if (!isSubString && !isSubObject) {
              result.errors.push({
                type: 'error',
                nodeId: node.id,
                nodeName: node.name,
                message: 'notes.sub must be a string or object value'
              });
            }
          }
        }
