 ...
 * [OUTPUT]: 对外提供 AilyChatComponent，对接旧 Agent/QA 模式与 Tesseract backend 的教学/对话双子模式，并在 Tesseract 模式旁路旧登录浮层，同时承接数字孪生组装完成后的自动工作流推进（confirm/upload）。
...
import { SkillCenterComponent, SkillCenterPrimaryView } from '../skill-center/skill-center.component';
...
export class AilyChatComponent implements OnDestroy {
  ...
  private skillCenterModalRef?: NzModalRef<SkillCenterComponent>;
...
`;
  ...
  private matchesDialogueHardwareRequirement(requirement: DialogueHardwareRequirement): boolean {
    return this.dialogueBridgeState.components.some((component) => {
  ...
  private openSkillCenter(
    view: SkillCenterPrimaryView,
...
`);
  ...
  private buildDialogueValidationMarkdown(event: DialogueHardwareEvent): string {
    const connectedComponents = this.dialogueBridgeState.components.length > 0
  ...
  private async uploadWorkflowToHardware(sessionId: string): Promise<{ markdown: string }> {
    ...
    return {
      markdown: this.buildAilyStateMarkdown(
        'done',
        result?.data?.statusText || result?.statusText || '已上传到硬件。'
      ),
    };
  }
  ...
  private async stopWorkflowOnHardware(sessionId: string): Promise<{ markdown: string }> {
    ...
    return {
      markdown: this.buildAilyStateMarkdown(
        'info',
        result?.data?.statusText || result?.statusText || '已停止工作流。'
      ),
    };
  }
...
`);
  ...
  private async runTesseractAction(action: string, data: any): Promise<void> {
    ...
    if (action === 'tesseract-assembly-completed-from-twin') {
      ...
      try {
        (window['electronAPI'] as any)?.window?.closeByRole?.('digital-twin-assembly');
      } catch { /* web 环境静默 */ }
    ...
    try {
      ...
      switch (action) {
        ...
        case 'tesseract-save-skill': {
          ...
          this.openSkillCenter('library', savedSkill.skillId);
          result = {
            markdown: this.buildAilyStateMarkdown(
              'done',
        ...
        case 'tesseract-publish-skill':
          this.openSkillCenter('market', data?.skillId || null);
          result = {
            markdown: this.buildAilyStateMarkdown(
              'info',
...
`);
  ...
  private async startHardwareAssembly(data: any): Promise<void> {
    ...
    // 打开数字孪生组装检测台
    const digitalTwinUrl = await this.resolveAssemblyDigitalTwinUrl();
    this.uiService.openWindow({
      path: `iframe?url=${encodeURIComponent(digitalTwinUrl)}`,
      title: '硬件组装检测台',
      keepAboveMain: true,
      windowRole: 'digital-twin-assembly',
      width: 1400,
      height: 900,
      data: {
  ...
  private async resolveAssemblyDigitalTwinUrl(): Promise<string> {
    const DEFAULT_URL = 'http://127.0.0.1:18082/';
    const env = window['env'];
    const keys = ['AILY_TESSERACT_FRONTEND_URL', 'TESSERACT_FRONTEND_URL'];

    for (const key of keys) {
      try {
        const val = await env?.get?.(key);
        const trimmed = val?.trim();
        if (trimmed) {
          const u = new URL(trimmed);
          u.searchParams.set('entry', 'digital-twin');
          u.searchParams.set('source', 'aily-blockly');
          return u.toString();
        }
      } catch {
        // 静默忽略，尝试下一个 key
