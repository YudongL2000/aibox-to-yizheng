/**
 * [INPUT]: 依赖项目/串口/UI 服务、顶部菜单配置与右侧支持层 payload 协议装配主窗口顶栏交互。
 * [OUTPUT]: 对外提供 HeaderComponent，渲染开发板选择、窗口控制，以及右上角“我的库 / 广场 / AI / 模型 / 串口”这一排一级页面入口；在 Tesseract Studio 路由下额外承载唯一一份 `Digital Twin / Workflow` 主任务切换，并收紧 macOS 左侧前导留白让主导航贴左对齐。
 * [POS]: main-window/components/header 的顶栏控制器，负责把全局项目操作、工作区主视图切换与支持层单页入口收束到 Electron 壳层，避免 studio/content pane 再复制第二层 header。
...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  private readonly primarySupportAppIds = ['aily-chat', 'model-store', 'serial-monitor'];
  private readonly tesseractPriorityIds = [
    'aily-chat',
    'model-store',
    'serial-monitor',
    'cloud-space',
  ...
  get isTesseractStudioRoute(): boolean {
    return this.router.url.indexOf('/main/tesseract-studio') !== -1;
  }

  get headerApps(): AppItem[] {
    if (!this.isTesseractStudioRoute) {
      return APP_LIST;
    }

    return [...APP_LIST].sort((left, right) => {
      const leftIndex = this.tesseractPriorityIds.indexOf(left.id);
      const rightIndex = this.tesseractPriorityIds.indexOf(right.id);
      const resolvedLeft = leftIndex === -1 ? 999 : leftIndex;
      const resolvedRight = rightIndex === -1 ? 999 : rightIndex;
  ...
  get showProjectTitle(): boolean {
    return !this.isTesseractStudioRoute;
  }

  get activeSkillPanelView(): SkillPanelViewId | null {
    if (!this.uiService.isToolOpen('support-panel')) {
      return null;
    }

    const payload = this.uiService.getSupportPanelState();
    if (payload?.view === 'skills-market') {
      return 'market';
    }
    if (payload?.view === 'skills-library') {
      return 'library';
    }
    return null;
  }

  get supportLauncherApps(): AppItem[] {
    return this.headerApps.filter((app) => this.primarySupportAppIds.includes(app.id));
  }

  get activeWorkbenchSurface(): 'digital-twin' | 'workflow' | null {
    if (!this.isTesseractStudioRoute) {
      return null;
    }

    return this.uiService.getActiveWorkbenchSurface();
  }

  get secondaryHeaderApps(): AppItem[] {
    if (this.isTesseractStudioRoute) {
      return [];
    }
    return this.headerApps.filter((app) => !this.primarySupportAppIds.includes(app.id));
  ...
  switchWorkbenchSurface(surface: 'digital-twin' | 'workflow'): void {
    if (!this.isTesseractStudioRoute) {
      return;
    }

    void this.uiService.openWorkbenchSurface(surface);
  }
  ...
  openSkillPanel(view: SkillPanelViewId): void {
  ...
  getSupportLauncherLabel(id: string): string {
    switch (id) {
      case 'aily-chat':
        return 'AI';
      case 'model-store':
  ...
  async process(item: IMenuItem, event = null) {
    switch (item.action) {
      case 'project-new':
        ...
        this.uiService.openModal('project-new');
        break;
      case 'project-open':
        if (this.isLoaded()) { // 只在已加载项目时检查
  ...
  */
  ...
  isLoaded() {
    ...
    for (const router of ['/main/blockly-editor', '/main/code-editor', '/main/tesseract-studio']) {
      if (this.router.url.indexOf(router) > -1) {
        return true;
      }
    }
