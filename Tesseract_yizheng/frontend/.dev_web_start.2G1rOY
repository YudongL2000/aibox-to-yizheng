#!/usr/bin/env bash
# [INPUT]: 依赖本地 Flutter SDK、Node.js、dev_web_proxy.js 与 18081/18082 端口可用性；Flutter 使用 --no-web-resources-cdn，避免从 gstatic 拉 CanvasKit（离线/内网可用）。
# [OUTPUT]: 对外提供一键开发启动入口，前台运行 Flutter web-server，后台托管同源代理。
# [POS]: 仓库根目录的单入口启动脚本，负责清理残留进程、统一端口并强制用户只走 18082。[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Optional override: FLUTTER_BIN=/path/to/flutter (defaults to `flutter` on PATH)
FLUTTER_BIN="${FLUTTER_BIN:-$(command -v flutter 2>/dev/null || true)}"
NODE_BIN="${NODE_BIN:-node}"
FLUTTER_HOST="${FLUTTER_HOST:-127.0.0.1}"
FLUTTER_PORT="${FLUTTER_PORT:-18081}"
PROXY_PORT="${PROXY_PORT:-18082}"
AGENT_API="${AGENT_API:-http://127.0.0.1:3006}"
LOCAL_DEVICE_EVENT_PREVIEW="${LOCAL_DEVICE_EVENT_PREVIEW:-false}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/.dart_tool/codex_dev_web}"
PROXY_LOG="$LOG_DIR/proxy.log"
FLUTTER_SERVER_URL="http://${FLUTTER_HOST}:${FLUTTER_PORT}"
APP_URL="http://${FLUTTER_HOST}:${PROXY_PORT}"
PACKAGE_CONFIG="$ROOT_DIR/.dart_tool/package_config.json"

proxy_pid=""
node_cmd=""
proxy_entry="$ROOT_DIR/dev_web_proxy.js"

log() {
  printf '[dev-web] %s\n' "$1"
}

require_command() {
  local command_name="$1"
  if ! command -v "$command_name" >/dev/null 2>&1; then
    log "缺少依赖: $command_name"
    exit 1
  fi
}

resolve_node_bin() {
  local candidate

  for candidate in \
    "$NODE_BIN" \
    "/mnt/c/Program Files/nodejs/node.exe"
  do
    if [[ "$candidate" == */* ]]; then
      if [[ -x "$candidate" ]]; then
        printf '%s\n' "$candidate"
        return 0
      fi
      continue
    fi

    if command -v "$candidate" >/dev/null 2>&1; then
      command -v "$candidate"
      return 0
    fi
  done

  return 1
}

resolve_proxy_entry() {
  if [[ "$node_cmd" == *.exe ]]; then
    wslpath -w "$ROOT_DIR/dev_web_proxy.js"
    return 0
  fi

  printf '%s\n' "$ROOT_DIR/dev_web_proxy.js"
}

find_listen_pids() {
  local port="$1"
  lsof -tiTCP:"$port" -sTCP:LISTEN 2>/dev/null || true
}

stop_port_listener() {
  local port="$1"
  local pids

  pids="$(find_listen_pids "$port")"
  if [[ -z "$pids" ]]; then
    return
  fi

  log "清理端口 ${port} 上的旧进程: ${pids//$'\n'/ }"
  kill $pids 2>/dev/null || true
  sleep 1

  pids="$(find_listen_pids "$port")"
  if [[ -n "$pids" ]]; then
    log "端口 ${port} 上仍有残留，执行强制结束"
    kill -9 $pids 2>/dev/null || true
  fi
}

cleanup() {
  if [[ -n "$proxy_pid" ]] && kill -0 "$proxy_pid" 2>/dev/null; then
    kill "$proxy_pid" 2>/dev/null || true
    wait "$proxy_pid" 2>/dev/null || true
  fi
}

package_config_needs_refresh() {
  if [[ ! -f "$PACKAGE_CONFIG" ]]; then
    return 0
  fi

  python3 - "$PACKAGE_CONFIG" "$HOME" <<'PY'
import json
import pathlib
import sys

package_config = pathlib.Path(sys.argv[1])
home = pathlib.Path(sys.argv[2]).resolve()

try:
    data = json.loads(package_config.read_text())
except Exception:
    sys.exit(0)

for pkg in data.get("packages", []):
    root_uri = pkg.get("rootUri", "")
    if not root_uri.startswith("file:///"):
        continue
    path = pathlib.Path(root_uri.removeprefix("file://")).resolve()
    try:
        path.relative_to(home)
    except ValueError:
        if str(path).startswith("/Users/"):
            sys.exit(0)

sys.exit(1)
PY
}

refresh_pub_get_if_needed() {
  if ! package_config_needs_refresh; then
    return
  fi

  log "检测到 .dart_tool/package_config.json 来自其他机器或已失效，先执行 flutter pub get"
  "$FLUTTER_BIN" pub get
}

trap cleanup EXIT INT TERM

require_command lsof
if ! node_cmd="$(resolve_node_bin)"; then
  log "缺少依赖: node / node.exe"
  log "可通过 NODE_BIN=/path/to/node 覆盖默认路径"
  exit 1
fi
# wslpath 仅 WSL 下配合 node.exe 需要；macOS / 原生 Linux 使用 node 时不必安装
if [[ "$node_cmd" == *.exe ]]; then
  require_command wslpath
fi
proxy_entry="$(resolve_proxy_entry)"

if [[ ! -x "$FLUTTER_BIN" ]]; then
  log "Flutter 不可执行: $FLUTTER_BIN"
  log "可通过 FLUTTER_BIN=/path/to/flutter 覆盖默认路径"
  exit 1
fi

mkdir -p "$LOG_DIR"
refresh_pub_get_if_needed

stop_port_listener "$PROXY_PORT"
stop_port_listener "$FLUTTER_PORT"

: >"$PROXY_LOG"
"$node_cmd" \
  "$proxy_entry" \
  "--port=$PROXY_PORT" \
  "--agent-api=$AGENT_API" \
  "--flutter-web-server=$FLUTTER_SERVER_URL" >"$PROXY_LOG" 2>&1 &
proxy_pid="$!"

sleep 1
if ! kill -0 "$proxy_pid" 2>/dev/null; then
  log "代理启动失败，日志如下:"
  cat "$PROXY_LOG"
  exit 1
fi

log "代理已启动: $APP_URL"
log "Flutter Web Server: $FLUTTER_SERVER_URL"
log "Agent API: $AGENT_API"
log "Node: $node_cmd"
log "代理日志: $PROXY_LOG"
log "浏览器只打开这个地址: $APP_URL"
log "Flutter 交互命令仍然可用，改默认场景后优先按大写 R"
log "本地设备预览: $LOCAL_DEVICE_EVENT_PREVIEW"

flutter_args=(
  run
  -d
  web-server
  # Serve CanvasKit/engine assets from the local SDK build instead of www.gstatic.com (offline / firewall safe).
  --no-web-resources-cdn
  --web-hostname
  "$FLUTTER_HOST"
  --web-port
  "$FLUTTER_PORT"
  --dart-define=USE_WEB_PROXY=true
  --dart-define=SKIP_LOGIN=true
  --dart-define=USE_LOCAL_DEVICE_EVENT_PREVIEW="$LOCAL_DEVICE_EVENT_PREVIEW"
  --no-pub
)

set +e
"$FLUTTER_BIN" "${flutter_args[@]}"
flutter_status=$?
set -e

exit "$flutter_status"
