/**
 * [INPUT]: 依赖 ChatService、ProjectService、TesseractChatService 与工具系统提供的会话、项目路径、backend Agent 和上下文能力。
 * [OUTPUT]: 对外提供 AilyChatComponent，对接旧 Agent/QA 模式与 Tesseract backend 的教学/对话双子模式，并支持嵌入 `Assistant` 支持层与独立子窗口两种呈现方式，同时按真实 receipt 展示硬件 workflow upload/stop 状态。
 * [POS]: tools/aily-chat 的主聊天容器，负责模式切换、消息流转、历史会话、对话模式硬件桥、MimicLaw relay、Tesseract 动作/工作区路由桥接与硬件组装阶段状态管理（014-hardware-assembly-twin-ux）。
 * [PROTOCOL]: 变更时更新此头部，然后检查 AGENTS.md
 */

import { Component, ElementRef, ViewChild, OnDestroy, ChangeDetectorRef, Input, OnInit } from '@angular/core';
...
import { SkillPanelViewId, UiService } from '../../services/ui.service';
...
import { TesseractChatService } from './services/tesseract-chat.service';
import {
  DialogueBridgeState,
...
import {
  ...
  DialogueHardwareRequirement,
  DialogueLibrarySkillPreview,
  DialogueRelayDirective,
...
export class AilyChatComponent implements OnInit, OnDestroy {
  ...
  private chatActionHandler: ((event: Event) => void) | null = null;
  private lastStopReason: string = ''; // 保存上次停止原因用于重试
  private readonly dialogueModeStorageKey = 'aily.tesseract.interactionMode';
  tesseractInteractionMode: TesseractInteractionMode = this.loadTesseractInteractionMode();
  dialogueBridgeState: DialogueBridgeState = {
    connected: false,
    connecting: false,
    url: '',
    lastError: null,
  ...
  private readonly mimiclawActivationAudio = 'magiclaw_activated.mp3';
  private readonly mimiclawDeactivationAudio = 'magiclaw_deactivated.mp3';
  private readonly dialogueAudioDataUrlCache = new Map<string, string>();
  private lastDialogueHardwareReady = false;
  private dialogueBridgeEventSubscription: Subscription | null = null;
  private dialogueBridgeStateSubscription: Subscription | null = null;
  private dialogueHardwareRuntimeSubscription: Subscription | null = null;
  private dialogueSkillsSubscription: Subscription | null = null;
  private backendHardwareRuntimeSnapshot: HardwareRuntimeSnapshot | null = null;
  private pendingTeachingContext: DialogueTeachingContext | null = null;
  private currentDialogueModeState: DialogueModeEnvelope | null = null;
  /** 教学/对话模式各自保存的上次会话 ID，用于模式切换时独立恢复上下文 */
  private savedTeachingSessionId: string | null = null;
  private savedDialogueSessionId: string | null = null;
  /** 对话模式顶部展示的技能卡片列表 */
  dialogueSkills: DialogueLibrarySkillPreview[] = [];
  private lastDialogueValidationAt = 0;

  // --------------------------------------------------------------------------
  // 014-hardware-assembly-twin-ux: 硬件组装阶段状态
  // --------------------------------------------------------------------------
  /** 当前硬件组装阶段 */
  dialogueAssemblyPhase: 'idle' | 'assembling' | 'complete' = 'idle';
  /** 当前正在组装的硬件组件列表（从 hot_plugging 响应 payload 中传入） */
  dialoguePendingComponents: Array<{ componentId: string; displayName: string }> = [];
  private assemblyMonitorSubscription: Subscription | null = null;
  ...
  get currentMode() {
    return this.getEffectiveChatMode();
  }
  ...
  get useLegacyLoginOverlay(): boolean {
    return this.currentMode !== 'tesseract';
  }

  get isTesseractProjectActive(): boolean {
    return this.resolveChatProjectMode() === 'tesseract';
  }

  get isDialogueSubMode(): boolean {
    return this.currentMode === 'tesseract' && this.tesseractInteractionMode === 'dialogue';
  }

  get isDialogueMimiclawConnected(): boolean {
    return this.hasConnectedMimiclawComponent(this.getEffectiveDialogueHardwareComponents());
  }

  get dialogueBridgeStatusLabel(): string {
    if (this.dialogueBridgeState.connecting && !this.isDialogueMimiclawConnected) {
      return '硬件桥连接中';
    }
    return this.isDialogueMimiclawConnected ? '磷虾U盘已连接' : '硬件桥未连接';
  }

  get hasDialogueSkills(): boolean {
    return this.isDialogueSubMode && this.dialogueSkills.length > 0;
  }

  get dialogueReadySkillCount(): number {
    return this.dialogueSkills.filter((skill) => this.isDialogueSkillReady(skill)).length;
  }
...
`;
  ...
  getCurrentProjectPath(): string {
    ...
    if (this.tesseractProjectService.getProjectMode(currentProjectPath) === 'tesseract') {
      return currentProjectPath;
    }

    return currentProjectPath !== this.projectService.projectRootPath
      ? currentProjectPath
      : '';
  }

  private isCurrentWorkspaceMatch(projectPath?: string | null): boolean {
    const expectedPath = String(projectPath || '').trim();
    const currentPath = String(this.getCurrentProjectPath() || '').trim();
    return expectedPath === currentPath;
  }

  private shouldSyncWorkflowIntoClientWorkspace(projectPath?: string | null): boolean {
    const targetPath = String(projectPath || '').trim();
    const currentPath = String(this.getCurrentProjectPath() || '').trim();
    if (!targetPath) {
      return !currentPath || this.isTesseractWorkspaceRoute();
    }

    return !currentPath || this.isCurrentWorkspaceMatch(targetPath);
  }

  private isTesseractWorkspaceRoute(): boolean {
    return this.router.url.indexOf('/main/tesseract-studio') !== -1;
  }

  private getEffectiveChatMode(): 'agent' | 'ask' | 'tesseract' {
    if (this.resolveChatProjectMode() === 'tesseract') {
      return 'tesseract';
    }

    return this.chatService.currentMode === 'ask' ? 'ask' : 'agent';
  }

  private resolveChatProjectMode(projectPath = this.projectService.currentProjectPath || ''): 'tesseract' | 'blockly' | 'code' {
    const currentProjectPath = projectPath || '';
    if (!currentProjectPath) {
      return this.isTesseractWorkspaceRoute() ? 'tesseract' : 'code';
    }

    return this.tesseractProjectService.getProjectMode(currentProjectPath);
  }

  private syncChatModeWithProject(projectPath: string): void {
    const mode = this.resolveChatProjectMode(projectPath);
    if (mode === 'tesseract' && this.chatService.currentMode !== 'tesseract') {
      this.chatService.saveChatMode('tesseract');
      this.chatService.currentSessionId = '';
      this.serverSessionActive = false;
      void this.syncDialogueBridgeWithMode();
      return;
    }
    ...
    if (mode !== 'tesseract' && this.chatService.currentMode === 'tesseract') {
      ...
      this.chatService.currentSessionId = '';
      this.serverSessionActive = false;
      this.currentDialogueModeState = null;
      this.pendingTeachingContext = null;
      void this.syncDialogueBridgeWithMode();
      return;
    }
  ...
  private loadTesseractInteractionMode(): TesseractInteractionMode {
    try {
      return window.localStorage.getItem(this.dialogueModeStorageKey) === 'dialogue'
        ? 'dialogue'
        : 'teaching';
    } catch (_error) {
      return 'teaching';
  ...
  private persistTesseractInteractionMode(mode: TesseractInteractionMode): void {
    try {
      window.localStorage.setItem(this.dialogueModeStorageKey, mode);
    } catch (_error) {
      // ignore storage errors
    }
  }

  private initializeDialogueBridgeSubscriptions(): void {
    if (!this.dialogueBridgeStateSubscription) {
      this.dialogueBridgeStateSubscription = this.dialogueHardwareBridgeService.state$.subscribe((state) => {
        this.dialogueBridgeState = state;
        this.syncDialogueHardwarePresenceFeedback();
        this.cdr.markForCheck();
      });
    }

    if (!this.dialogueHardwareRuntimeSubscription) {
      this.hardwareRuntimeService.start();
      this.dialogueHardwareRuntimeSubscription = this.hardwareRuntimeService.status$.subscribe((snapshot) => {
        this.backendHardwareRuntimeSnapshot = snapshot;
        this.syncDialogueHardwarePresenceFeedback();
        this.cdr.markForCheck();
      });
    }

    if (!this.dialogueBridgeEventSubscription) {
      this.dialogueBridgeEventSubscription = this.dialogueHardwareBridgeService.events$.subscribe((event) => {
        void this.handleDialogueHardwareEvent(event);
      });
    }
  }

  private initializeDialogueSkillSubscription(): void {
    if (this.dialogueSkillsSubscription) {
      return;
    }

    this.dialogueSkillsSubscription = this.tesseractSkillLibraryService.skills$.subscribe((skills) => {
      this.dialogueSkills = skills;
      this.cdr.markForCheck();
    });
  }

  private async refreshDialogueSkills(forceLoad: boolean = false): Promise<void> {
    if (!this.isDialogueSubMode) {
      this.dialogueSkills = [];
      this.cdr.markForCheck();
      return;
    }

    const shouldLoad = forceLoad || this.tesseractSkillLibraryService.snapshot.length === 0;
    if (!shouldLoad) {
      this.dialogueSkills = this.tesseractSkillLibraryService.snapshot;
      this.cdr.markForCheck();
      return;
    }

    try {
      this.dialogueSkills = await this.tesseractSkillLibraryService.load(this.getCurrentProjectPath());
    } catch (_error) {
      this.dialogueSkills = this.tesseractSkillLibraryService.snapshot;
    }
  ...
  private async syncDialogueBridgeWithMode(): Promise<void> {
    if (this.currentMode === 'tesseract' && this.tesseractInteractionMode === 'dialogue') {
      await this.dialogueHardwareBridgeService.ensureConnected();
      return;
    }

    this.dialogueHardwareBridgeService.disconnect();
  }

  setTesseractInteractionMode(mode: TesseractInteractionMode): void {
    if (this.tesseractInteractionMode === mode) {
      return;
    }

    // 保存当前模式的会话 ID，提交脏数据到磁盘
    this.saveCurrentSession();
    if (this.tesseractInteractionMode === 'teaching') {
      this.savedTeachingSessionId = this.sessionId || null;
    } else {
      this.savedDialogueSessionId = this.sessionId || null;
    }

    this.tesseractInteractionMode = mode;
    this.persistTesseractInteractionMode(mode);

    // 切换到目标模式：恢复上次该模式的会话，或开启独立新会话
    const targetSavedId = mode === 'teaching'
      ? this.savedTeachingSessionId
      : this.savedDialogueSessionId;

    if (targetSavedId) {
      this.chatService.currentSessionId = targetSavedId;
      this.getHistory();
    } else {
      // 首次进入该模式：清空上下文，确保不污染跨模式 session
      this.chatService.currentSessionId = '';
      this.list = [];
    ...
    if (mode !== 'dialogue') {
      ...
      void this.refreshDialogueSkills(true);
    }
    void this.syncDialogueBridgeWithMode();
  }

  getDialogueSkillPrompt(skill: DialogueLibrarySkillPreview): string {
    return String(skill?.wakePrompt || skill?.displayName || '').trim();
  }

  getDialogueSkillHardwareStates(skill: DialogueLibrarySkillPreview): Array<{ label: string; ready: boolean }> {
    const requirements = Array.isArray(skill?.requiredHardware) ? skill.requiredHardware : [];
    return requirements.slice(0, 3).map((requirement) => ({
      label: String(requirement?.displayName || requirement?.componentId || '硬件').trim(),
      ready: this.matchesDialogueHardwareRequirement(requirement),
    }));
  }

  isDialogueSkillReady(skill: DialogueLibrarySkillPreview): boolean {
    const requirements = Array.isArray(skill?.requiredHardware) ? skill.requiredHardware : [];
    if (requirements.length === 0) {
      return true;
    }

    return requirements.every((requirement) => this.matchesDialogueHardwareRequirement(requirement));
  }

  private matchesDialogueHardwareRequirement(requirement: DialogueHardwareRequirement): boolean {
    return this.getEffectiveDialogueHardwareComponents().some((component) => {
      if (!component || component.status === 'removed' || component.status === 'error') {
        return false;
      }

      return this.collectDialogueHardwareTokens(requirement).some((token) =>
        this.collectDialogueHardwareTokens(component).includes(token)
      );
    });
  }

  private isDialogueBridgeReady(state: DialogueBridgeState): boolean {
    return Boolean(state.connected) && this.hasConnectedMimiclawComponent(state.components);
  }

  private syncDialogueHardwarePresenceFeedback(): void {
    const isReady = this.isDialogueMimiclawConnected;
    if (this.lastDialogueHardwareReady !== isReady && this.isDialogueSubMode) {
      void this.playDialogueBridgePresenceAudio(isReady);
    }
    this.lastDialogueHardwareReady = isReady;
  }

  private getEffectiveDialogueHardwareComponents(
    ...fallbackGroups: DialogueHardwareComponent[][]
  ): DialogueHardwareComponent[] {
    return this.mergeDialogueHardwareComponents(
      this.dialogueBridgeState.components,
      this.extractRuntimeDialogueComponents(this.backendHardwareRuntimeSnapshot),
      ...fallbackGroups,
    );
  }

  private mergeDialogueHardwareComponents(
    ...groups: DialogueHardwareComponent[][]
  ): DialogueHardwareComponent[] {
    ...
    for (const group of groups) {
      for (const component of group || []) {
        ...
        next.set(this.dialogueHardwareComponentKey(component), component);
      }
    }
    return Array.from(next.values());
  }

  private dialogueHardwareComponentKey(component: DialogueHardwareComponent): string {
    return [
      String(component.componentId || '').trim().toLowerCase(),
      String(component.deviceId || '').trim().toLowerCase(),
      String(component.portId || '').trim().toLowerCase(),
    ].join(':');
  }

  private extractRuntimeDialogueComponents(
    snapshot: HardwareRuntimeSnapshot | null,
  ): DialogueHardwareComponent[] {
    const connectedComponents = snapshot?.hardware?.dialogueHardware?.['connectedComponents'];
    if (!Array.isArray(connectedComponents)) {
      return [];
    }

    return connectedComponents
      .map((component) => this.normalizeDialogueHardwareComponent(component))
      .filter((component): component is DialogueHardwareComponent => Boolean(component));
  }

  private normalizeDialogueHardwareComponent(raw: unknown): DialogueHardwareComponent | null {
    if (!raw || typeof raw !== 'object') {
      return null;
    }

    const component = raw as Record<string, unknown>;
    const componentId = this.readDialogueHardwareString(component['componentId'] ?? component['component_id']);
    const deviceId = this.readDialogueHardwareString(component['deviceId'] ?? component['device_id']);
    ...
    return {
      componentId,
      deviceId,
      modelId: this.readDialogueHardwareString(component['modelId'] ?? component['model_id']) || `${componentId}-001`,
      displayName: this.readDialogueHardwareString(component['displayName'] ?? component['display_name']) || deviceId,
      portId: this.readDialogueHardwareString(component['portId'] ?? component['port_id']) || '',
      status: this.normalizeDialogueHardwareStatus(component['status']),
    };
  }

  private readDialogueHardwareString(value: unknown): string {
    return typeof value === 'string' ? value.trim() : '';
  }

  private normalizeDialogueHardwareStatus(value: unknown): DialogueHardwareComponent['status'] {
    const normalized = this.readDialogueHardwareString(value).toLowerCase();
    if (!normalized) {
      return 'connected';
    }
    if (normalized.includes('remove') || normalized.includes('offline') || normalized.includes('disconnect')) {
      return 'removed';
    }
  ...
  private hasConnectedMimiclawComponent(components: DialogueHardwareComponent[]): boolean {
    return components.some((component) => {
      if (!component || component.status === 'removed' || component.status === 'error') {
        return false;
      }
      return this.collectDialogueHardwareTokens(component).some((token) => this.isMimiclawToken(token));
    });
  }

  private isMimiclawToken(token: string): boolean {
    const normalized = String(token || '').trim().toLowerCase();
    if (!normalized) {
      return false;
    }
    return (
      /^mimic_?law\d*$/.test(normalized) ||
      /^magiclaw\d*$/.test(normalized) ||
      normalized.includes('磷虾')
    );
  }

  private async playDialogueBridgePresenceAudio(connected: boolean): Promise<void> {
    if (!this.isDialogueSubMode) {
      return;
    }

    const fileName = connected
      ? this.mimiclawActivationAudio
      : this.mimiclawDeactivationAudio;
    const audioDataUrl = await this.resolveDialogueAudioDataUrl(fileName);
    if (!audioDataUrl) {
      return;
    }
    ...
    try {
      ...
      console.warn('[DialogueBridge] 播放磷虾U盘状态音失败', error);
    }
  }

  private async resolveDialogueAudioDataUrl(fileName: string): Promise<string | null> {
    const cached = this.dialogueAudioDataUrlCache.get(fileName);
    if (cached) {
      return cached;
    }

    const electronApi = (window as any)?.electronAPI;
    if (!electronApi?.path || !electronApi?.fs?.readFileAsBase64) {
      return null;
    }

    try {
      const electronDir = electronApi.path.getElectronPath();
      const appRoot = electronApi.path.dirname(electronApi.path.dirname(electronDir));
      const audioPath = electronApi.path.join(
        appRoot,
        'backend',
        'data',
        'audios',
      ...
      if (!electronApi.fs.existsSync(audioPath)) {
        return null;
      }
      const base64 = electronApi.fs.readFileAsBase64(audioPath);
      const dataUrl = `data:audio/mpeg;base64,${base64}`;
      this.dialogueAudioDataUrlCache.set(fileName, dataUrl);
      return dataUrl;
    } catch (error) {
      console.warn('[DialogueBridge] 读取磷虾U盘状态音失败', error);
      return null;
    }
  }

  private collectDialogueHardwareTokens(source: Partial<DialogueHardwareRequirement & DialogueHardwareComponent>): string[] {
    const values = [source.componentId, source.displayName, source.deviceId, source.modelId];
    ...
    for (const value of values) {
      ...
      this.expandDialogueHardwareAliases(normalized).forEach((alias) => tokens.add(alias));
    }

    return Array.from(tokens);
  }

  private expandDialogueHardwareAliases(token: string): string[] {
    switch (token) {
      case 'cam':
      case 'camera':
      case 'webcam':
      ...
      case '底盘':
      case '小车':
        return ['wheel', 'chassis', 'base', '底盘', '小车'];
      case 'mechanical_hand':
      case 'hand':
      case 'claw':
      case '夹爪':
      case '机械手':
        return ['mechanical_hand', 'hand', 'claw', '夹爪', '机械手'];
      case 'speaker':
      case '喇叭':
      case '扬声器':
        return ['speaker', '喇叭', '扬声器'];
      default:
        return [token];
    }
  }

  async sendDialogueQuickPrompt(prompt: string): Promise<void> {
    const normalizedPrompt = String(prompt || '').trim();
    if (!normalizedPrompt || this.isWaiting) {
      return;
    }

    if (this.tesseractInteractionMode !== 'dialogue') {
      this.setTesseractInteractionMode('dialogue');
    }

    this.inputValue = normalizedPrompt;
    await this.send('user', normalizedPrompt, true);
  }

  private openTeachingFromDialogue(data: any): void {
    const teachingContext = data?.teachingContext || null;
    const prefilledGoal = String(
      data?.prefilledGoal
      || teachingContext?.prefilledGoal
      || teachingContext?.originalPrompt
      || ''
    ).trim();

    this.pendingTeachingContext = teachingContext;
    this.setTesseractInteractionMode('teaching');
    this.currentDialogueModeState = null;
    this.inputValue = prefilledGoal;
  ...
  private openHistorySession(sessionId: string | null | undefined): void {
    const normalizedSessionId = String(sessionId || '').trim();
    if (!normalizedSessionId || this.chatService.currentSessionId === normalizedSessionId) {
      this.closeMenu();
      return;
    }

    this.saveCurrentSession();
    this.chatService.currentSessionId = normalizedSessionId;
    const entry = this.chatHistoryService.findEntry(normalizedSessionId);
  ...
  openSkillPanel(
    view: SkillPanelViewId,
    highlightSkillId?: string | null
  ): void {
    this.uiService.openSupportPanel({
      view: view === 'market' ? 'skills-market' : 'skills-library',
      highlightSkillId: highlightSkillId || null,
    });
  }

  private buildAilyStateMarkdown(state: 'info' | 'done' | 'error', text: string): string {
    return [
      '```aily-state',
  ...
  private async openWorkflowInClientWorkspace(
    payload: {
    ...
    const projectPath = String(payload?.projectPath || this.getCurrentProjectPath() || '').trim();
  ...
  private async applyDialogueResponseSideEffects(
    response: any
  ): Promise<{ overrideMarkdown?: string | null; source?: string }> {
    if (response?.type !== 'dialogue_mode') {
      this.currentDialogueModeState = null;
      this.cdr.markForCheck();
      return {};
    }
    ...
    this.currentDialogueModeState = dialogueMode;
    this.cdr.markForCheck();

    let overrideMarkdown: string | null = null;
    if (dialogueMode?.relay?.target === 'mimiclaw_ws') {
      overrideMarkdown = await this.relayDialogueToMimiclaw(dialogueMode.relay);
    }

    if (dialogueMode?.physicalCue?.autoTrigger) {
      await this.dialogueHardwareBridgeService.ensureConnected();
      this.dialogueHardwareBridgeService.dispatchPhysicalCue(dialogueMode.physicalCue, this.sessionId);
    }
  ...
  private async handleDialogueHardwareEvent(event: DialogueHardwareEvent): Promise<void> {
    if (!this.isDialogueSubMode || !this.sessionId || this.isWaiting || !this.currentDialogueModeState) {
      return;
    }

    if (!['device_inserted', 'device_removed', 'device_error', 'device_ready', 'snapshot'].includes(event.eventType)) {
      return;
    }

    const now = Date.now();
    if (now - this.lastDialogueValidationAt < 600) {
      return;
    }
    this.lastDialogueValidationAt = now;

    this.isWaiting = true;
    this.isCompleted = false;
    this.appendMessage('aily', this.buildDialogueValidationMarkdown(event), 'mainAgent');

    try {
      const result = await this.tesseractChatService.validateDialogueHardware(
        this.sessionId,
        event,
        this.getCurrentProjectPath()
      );
      if (result?.sessionId) {
        this.chatService.currentSessionId = result.sessionId;
      }
      await this.applyDialogueResponseSideEffects(result?.response);
      this.isWaiting = false;
      this.isCompleted = true;
      this.replaceLastAilyMessage(result?.markdown || '硬件校验已完成');
      this.saveCurrentSession();
      this.refreshHistoryList();
    } catch (error: any) {
      this.isWaiting = false;
      this.isCompleted = true;
      this.replaceLastAilyMessage(`
\`\`\`aily-state
${JSON.stringify({ state: 'error', text: error?.message || '硬件校验失败，请检查接口后重试' }, null, 2)}
\`\`\`
`);
    }
  }

  private buildDialogueValidationMarkdown(event: DialogueHardwareEvent): string {
    const connectedComponents = this.getEffectiveDialogueHardwareComponents(
      event.connectedComponents && event.connectedComponents.length > 0
        ? event.connectedComponents
        : event.component
          ? [event.component]
          : this.currentDialogueModeState?.hardware.connectedComponents || [],
    );
    const validationText = event.eventType === 'device_removed'
      ? '检测到硬件被拔出，正在重新校验状态...'
    ...
    return [
      ...
      JSON.stringify({
        ...
        branch: this.currentDialogueModeState?.branch || 'hardware_guidance',
        phase: 'validating_insert',
        message: this.currentDialogueModeState?.skill?.gameplayGuide || '我收到新的硬件信号了，正在确认它是否已经准备好。',
        skills: [],
        skill: this.currentDialogueModeState?.skill || null,
        hardware: {
          ...(this.currentDialogueModeState?.hardware || {}),
          connectedComponents,
          validationStatus: 'pending',
        },
        actions: [],
        physicalCue: null,
        teachingHandoff: this.currentDialogueModeState?.teachingHandoff || null,
        deploymentPrompt: null,
        localStatusText: validationText,
      }, null, 2),
      '```',
    ].join('\n');
  }

  private async relayDialogueToMimiclaw(
    relay: DialogueRelayDirective
  ): Promise<string> {
    try {
      const result = await this.dialogueHardwareBridgeService.relayChatMessage(relay.message, {
        chatId: relay.chatId,
      });
      const visibleMessages = result.messages.filter(
        (item) => !this.isMimiclawProgressText(item.content)
      );
      const content = (visibleMessages.length > 0 ? visibleMessages : result.messages)
        .map((item) => item.content)
        .join('\n\n');
      return this.formatMimiclawMarkdown(content);
    } catch (error: any) {
      return [
        '```aily-state',
        JSON.stringify({
          state: 'error',
          text: error?.message || 'MimicLaw 对话转发失败',
        }, null, 2),
        '```',
      ].join('\n');
    }
  ...
  private async uploadWorkflowToHardware(sessionId: string): Promise<{ markdown: string }> {
    if (!window['electronAPI']?.tesseract?.hardwareUpload) {
      return {
        markdown: this.buildAilyStateMarkdown('info', '当前环境不支持上传到硬件。'),
      };
    }
  ...
  private async stopWorkflowOnHardware(sessionId: string): Promise<{ markdown: string }> {
    if (!window['electronAPI']?.tesseract?.hardwareStop) {
      return {
        markdown: this.buildAilyStateMarkdown('info', '当前环境不支持停止硬件工作流。'),
      };
    }
  ...
  private buildHardwareReceiptMarkdown(
    result: any,
    action: 'workflow_upload' | 'workflow_stop'
  ): string {
    const fallbackError = action === 'workflow_upload' ? '工作流上传失败。' : '停止工作流失败。';
    if (!result?.success) {
      return this.buildAilyStateMarkdown('error', result?.error || fallbackError);
    }
    ...
    if (action === 'workflow_upload') {
      if (status === 'acknowledged' && responseStatus === 'started') {
        return this.buildAilyStateMarkdown(
          'done',
          workflowFile
            ? `工作流已下发到硬件并启动，端侧文件：${workflowFile}`
            : '工作流已下发到硬件并启动。'
        );
      }
      if (status === 'acknowledged') {
        return this.buildAilyStateMarkdown(
          'done',
          responseMessage || '硬件已确认收到工作流。'
        );
      }
      if (status === 'sent') {
        return this.buildAilyStateMarkdown('info', '工作流已发送到硬件，正在等待设备确认。');
      }
      if (status === 'queued') {
        const isDisabled = receipt?.requestId === 'disabled' || !receipt?.topic;
        return this.buildAilyStateMarkdown(
          isDisabled ? 'error' : 'info',
          isDisabled ? '后端未启用硬件下发能力，请先检查 MQTT/运行时配置。' : '工作流已入队，等待硬件链路可用。'
        );
      }
      if (status === 'failed') {
        return this.buildAilyStateMarkdown('error', responseMessage || '工作流下发失败，请检查后端日志。');
      }
    }

    if (action === 'workflow_stop') {
      if (status === 'acknowledged') {
        return this.buildAilyStateMarkdown('done', responseMessage || '硬件已确认停止工作流。');
      }
      if (status === 'sent') {
        return this.buildAilyStateMarkdown('info', '停止指令已发送，正在等待硬件确认。');
      }
      if (status === 'queued') {
        const isDisabled = receipt?.requestId === 'disabled' || !receipt?.topic;
        return this.buildAilyStateMarkdown(
          isDisabled ? 'error' : 'info',
          isDisabled ? '后端未启用硬件停止能力，请先检查 MQTT/运行时配置。' : '停止指令已入队，等待硬件链路可用。'
        );
      }
      if (status === 'failed') {
        return this.buildAilyStateMarkdown('error', responseMessage || '停止工作流失败，请检查后端日志。');
      }
    }

    return this.buildAilyStateMarkdown('info', responseMessage || fallbackError);
  }

  private formatMimiclawMarkdown(content: string): string {
    const normalized = String(content || '').trim();
    return normalized || 'MimiClaw 暂时没有返回内容。';
  }

  private isMimiclawProgressText(content: string): boolean {
...
`
  ...
  constructor(
    ...
    private tesseractSkillLibraryService: TesseractSkillLibraryService,
  ...
  ngOnInit() {
    ...
    this.syncChatModeWithProject(this.projectService.currentProjectPath);
    this.initializeDialogueBridgeSubscriptions();
    this.initializeDialogueSkillSubscription();
    void this.syncDialogueBridgeWithMode();
    void this.refreshDialogueSkills(this.tesseractInteractionMode === 'dialogue');
    ...
    if (this.useLegacyLoginOverlay) {
      this.authService.initializeAuth().then((_res) => {
        // 初始化后立即订阅
    ...
    this.chatActionHandler = this.handleChatAction.bind(this);
    document.addEventListener('aily-chat-action', this.chatActionHandler);
    ...
    this.projectPathSubscription = this.projectService.currentProjectPath$.pipe(
      ...
      (newPath: string) => {
        ...
        // 根据新的项目路径重新加载聊天历史
        this.refreshHistoryList();
        this.syncChatModeWithProject(newPath);
  ...
  async close() {
    // 最后关闭工具窗口
    this.uiService.closeTool(this.embedded ? 'support-panel' : 'aily-chat');
  }
  ...
  private replaceLastAilyMessage(text: string, source: string = 'mainAgent'): void {
    if (this.list.length > 0 && this.list[this.list.length - 1].role === 'aily') {
      this.list[this.list.length - 1].content = text;
      this.list[this.list.length - 1].state = this.isWaiting ? 'doing' : 'done';
      this.list[this.list.length - 1].source = source;
    } else {
      this.list.push({
        role: 'aily',
        content: text,
        state: this.isWaiting ? 'doing' : 'done',
        source,
      });
    }
  ...
  async startSession(): Promise<void> {
    ...
    if (this.currentMode === 'tesseract') {
      ...
      this.currentDialogueModeState = null;
      this.chatService.currentSessionId =
        this.chatService.currentSessionId || `tesseract-${Date.now()}`;
      this.chatService.currentSessionTitle = this.chatService.currentSessionTitle || '';
      this.chatService.currentSessionPath = this.getCurrentProjectPath();
      this.isSessionStarting = false;
  ...
  async send(sender: string, content: string, clear: boolean = true): Promise<void> {
    ...
    if (this.isCompleted) {
      ...
      if (this.useStatelessMode) {
        ...
        if (!this.serverSessionActive && this.currentMode !== 'tesseract') {
    ...
    if ((!this.sessionId && this.currentMode !== 'tesseract') || !text) return;
    ...
    if (sender === 'user') {
      ...
      } else if (!this.chatService.currentSessionTitle) {
        this.chatService.currentSessionTitle = content.trim().slice(0, 32);
      }
      ...
      if (this.currentMode === 'tesseract') {
        ...
        try {
          const teachingContext = this.pendingTeachingContext;
          const result = await this.tesseractChatService.chat(
            text,
            this.sessionId,
            this.getCurrentProjectPath(),
            {
              interactionMode: this.tesseractInteractionMode,
              teachingContext,
            }
          );
          if (result.sessionId) {
          ...
          this.serverSessionActive = true;
          this.chatService.currentSessionPath = this.getCurrentProjectPath();
          const sideEffects = await this.applyDialogueResponseSideEffects(result.response);
          this.isWaiting = false;
          this.isCompleted = true;
          this.replaceLastAilyMessage(
            sideEffects.overrideMarkdown || result.markdown || '已收到 Tesseract Agent 响应',
            sideEffects.source || 'mainAgent'
          );
          this.saveCurrentSession();
          this.refreshHistoryList();
        } catch (error: any) {
          ...
          this.replaceLastAilyMessage(`
\`\`\`aily-state
${JSON.stringify({ state: 'error', text: error?.message || 'Tesseract Agent 请求失败' }, null, 2)}
\`\`\`
`);
        }
        return;
...
`);
  ...
  get ModeList(): IMenuItem[] {
    ...
    if (this.isTesseractProjectActive) {
      modes.unshift({
        name: 'Tesseract',
        action: 'tesseract-mode',
  ...
  private handleChatAction(event: Event): void {
    const customEvent = event as CustomEvent;
    const { action, data } = customEvent.detail || {};
    if (!action || !String(action).startsWith('tesseract-')) {
      return;
    }

    this.runTesseractAction(action, data);
  }

  private async runTesseractAction(action: string, data: any): Promise<void> {
    if (action === 'tesseract-dialogue-send-prompt' || action === 'tesseract-dialogue-skill-prompt') {
      await this.sendDialogueQuickPrompt(data?.prompt || '');
      return;
    }

    if (action === 'tesseract-start-hardware-assembly') {
      await this.startHardwareAssembly(data);
      return;
    }

    // 016-twin-assembly-checklist: Flutter 数字孪生清单完成通知
    if (action === 'tesseract-assembly-completed-from-twin') {
      ...
      const result = await this.confirmHotplugNodeAndContinue(
        ...
        data?.nodeName,
        Array.isArray(data?.allPendingHardwareNodeNames) ? data.allPendingHardwareNodeNames : [],
      );
      await this.maybeTriggerWorkflowDeployAfterAssembly(result, data);
      return;
    }

    if (action === 'tesseract-hotplug-timeout') {
      // 组装模式下由 Flutter 数字孪生清单面板管理完成检测，忽略来自 config-guide 卡片的超时事件
      if (this.dialogueAssemblyPhase === 'assembling') return;
      this.appendMessage('aily', this.buildAilyStateMarkdown(
        'info',
        '热插拔等待超时。你可以检查硬件连接后继续等待，或点击下方“标记节点已处理”先跳过此步。'
      ));
      this.saveCurrentSession();
      return;
    }

    if (action === 'tesseract-dialogue-open-teaching-mode') {
      this.openTeachingFromDialogue(data);
      return;
    }

    if (action === 'tesseract-open-history-session') {
      this.openHistorySession(data?.sessionId);
      return;
    }
    ...
    try {
      ...
      switch (action) {
        case 'tesseract-confirm-workflow': {
          const confirmResult = await this.tesseractChatService.confirmWorkflow(sessionId, projectPath);

          // ── 020-workflow-gen-resilience: 确认未生成工作流时直接返回，不链入部署 ──
          const confirmType = confirmResult?.response?.type;
          if (confirmType !== 'workflow_ready') {
            result = confirmResult;
            break;
          }

          this.replaceLastAilyMessage(
            this.buildAilyStateMarkdown(
              'info',
              '工作流已确认，正在创建 backend 工作流并进入配置阶段...'
            ),
            'mainAgent'
          );
          this.cdr.markForCheck();

          const deployResult = await this.tesseractChatService.deployWorkflowAndStartConfigWithOptions(
            sessionId,
            projectPath,
            {
              workflow: confirmResult?.response?.workflow || null,
              source: 'chat-confirm',
            }
          );
          const shouldAutoUpload = data?.autoUploadAfterConfigComplete === true;
          const autoUploadResult = shouldAutoUpload && deployResult?.response?.type === 'config_complete'
            ? await this.uploadWorkflowToHardware(sessionId)
            : null;
          result = {
            ...deployResult,
            markdown: [confirmResult?.markdown, deployResult?.markdown, autoUploadResult?.markdown].filter(Boolean).join('\n\n'),
          };
          if (this.shouldSyncWorkflowIntoClientWorkspace(projectPath)) {
            await this.openWorkflowInClientWorkspace({
              projectPath,
              workflowId:
                deployResult?.workflowRef?.workflowId
                || deployResult?.snapshot?.workflowId
                || deployResult?.workflowId
                || deployResult?.response?.metadata?.workflowId
                || null,
              workflowUrl:
                deployResult?.workflowRef?.workflowUrl
        ...
        case 'tesseract-deploy-workflow':
          result = await this.tesseractChatService.deployWorkflowAndStartConfig(sessionId, projectPath);
          if (this.shouldSyncWorkflowIntoClientWorkspace(projectPath)) {
            await this.openWorkflowInClientWorkspace({
              projectPath,
              workflowId:
                result?.workflowRef?.workflowId
                || result?.snapshot?.workflowId
                || result?.workflowId
                || result?.response?.metadata?.workflowId
                || data?.workflowId
                || null,
              workflowUrl:
                result?.workflowRef?.workflowUrl
                || result?.workflowUrl
                || null,
            });
          }
          break;
          case 'tesseract-dialogue-run-skill':
            result = await this.tesseractChatService.deployWorkflowAndStartConfigWithOptions(
              sessionId,
              projectPath,
              {
            ...
            break;
        case 'tesseract-save-skill': {
          const savedSkill = await this.tesseractSkillLibraryService.save(sessionId, projectPath);
          this.openSkillPanel('library', savedSkill.skillId);
          result = {
            markdown: this.buildAilyStateMarkdown(
              'done',
              `已将“${savedSkill.displayName}”存入 Skills 库。`
            ),
          };
          break;
        }
        case 'tesseract-dismiss-save-skill':
          result = {
            markdown: this.buildAilyStateMarkdown('info', '本次教学成果暂不存入 Skills 库。'),
          };
          break;
        case 'tesseract-publish-skill':
          this.openSkillPanel('market', data?.skillId || null);
          result = {
            markdown: this.buildAilyStateMarkdown(
              'info',
              `“${data?.displayName || '当前技能'}”的发布入口已切换到 Skill 广场，后续接入真实后端即可完成上架。`
            ),
          };
          break;
        case 'tesseract-dialogue-start-deploy':
          result = await this.tesseractChatService.startDialogueDeploy(sessionId, projectPath);
          break;
        case 'tesseract-upload-to-hardware':
          result = await this.uploadWorkflowToHardware(sessionId);
          break;
        case 'tesseract-stop-workflow':
          result = await this.stopWorkflowOnHardware(sessionId);
          break;
        case 'tesseract-confirm-node':
          result = await this.tesseractChatService.confirmNode({
            sessionId,
            nodeName: data?.nodeName,
            portId: data?.portId,
            topology: data?.topology,
            device_ID: data?.device_ID,
            TTS_input: data?.TTS_input,
            execute_emoji: data?.execute_emoji,
            sub: data?.sub,
          }, projectPath);
          break;
        case 'tesseract-submit-config-interaction': {
          if (data?.type === 'image_upload') {
            const uploadResult = await this.tesseractChatService.uploadFaceImage({
              profile: data?.profile,
              fileName: data?.fileName,
              contentBase64: data?.contentBase64,
            }, projectPath);

            // ── 021-mqtt-image-upload: MQTT 发送失败时提前返回错误 ──
            if (uploadResult?.success === false) {
              result = {
                markdown: this.buildAilyStateMarkdown('error',
                  uploadResult?.error || '图片发送失败，请检查硬件连接'),
              };
              break;
            }

            const confirmResult = await this.tesseractChatService.confirmNode({
              sessionId,
              nodeName: data?.nodeName,
              sub: {
                face_info: uploadResult?.profile || data?.profile,
                // face_url 已移除：图片直接由 MQTT 发送至设备，无本地 URL
              },
            }, projectPath);
            result = {
              ...confirmResult,
              markdown: [
                this.buildAilyStateMarkdown('done', `已发送人脸样本至设备：${uploadResult?.profile || data?.profile}`),
                confirmResult?.markdown,
              ].filter(Boolean).join('\n\n'),
            };
            break;
          }

          result = await this.tesseractChatService.chat(
            data?.agentMessage || data?.message || '',
            sessionId,
            projectPath,
            {
              interactionMode: this.tesseractInteractionMode,
              ...(data?.clarificationCategory
                ? { clarificationContext: { category: data.clarificationCategory } }
                : {}),
            },
          );
          break;
        }
        case 'tesseract-open-workflow':
          if (await this.openWorkflowInClientWorkspace({
            projectPath: data?.projectPath || projectPath,
            workflowId: data?.workflowId || data?.workflowRef?.workflowId || null,
            workflowUrl: data?.workflowUrl || data?.workflowRef?.workflowUrl || null,
          })) {
            this.isWaiting = false;
            this.isCompleted = true;
            this.replaceLastAilyMessage(`
\`\`\`aily-state
${JSON.stringify({ state: 'done', text: '已切换到客户端工作流界面' }, null, 2)}
\`\`\`
`);
            return;
          }

          await this.tesseractChatService.openWorkflow({
            sessionId,
          ...
          this.replaceLastAilyMessage(`
\`\`\`aily-state
${JSON.stringify({ state: 'done', text: '已在外部窗口打开工作流' }, null, 2)}
\`\`\`
`);
          return;
        default:
      ...
      this.isCompleted = true;
      this.replaceLastAilyMessage(
        sideEffects.overrideMarkdown || result?.markdown || 'Tesseract 动作已执行完成',
        sideEffects.source || 'mainAgent'
      );
      this.saveCurrentSession();
      this.refreshHistoryList();
      ...
      this.replaceLastAilyMessage(`
\`\`\`aily-state
${JSON.stringify({ state: 'error', text: error?.message || 'Tesseract 动作执行失败' }, null, 2)}
\`\`\`
`);
    }
  }
  ...
  // --------------------------------------------------------------------------
  // 014-hardware-assembly-twin-ux: 硬件组装阶段私有方法
  // --------------------------------------------------------------------------

  private async startHardwareAssembly(data: any): Promise<void> {
    const sessionId = data?.sessionId || this.sessionId;
    const components: Array<{ componentId: string; displayName: string }> = data?.components || [];
    const nodeName: string | undefined = data?.nodeName;
    const allPendingHardwareNodeNames: string[] = Array.isArray(data?.allPendingHardwareNodeNames)
      ? data.allPendingHardwareNodeNames
      : [];
    ...
    this.dialogueAssemblyPhase = 'assembling';
    this.dialoguePendingComponents = components;
    this.cdr.markForCheck();

    const assemblySession = this.assemblyOrchestrator.startSession({
    ...
    this.uiService.openWorkbenchPanel('digital-twin', assemblySession as unknown as Record<string, unknown>);

    // 在聊天中追加状态提示
    this.appendMessage('aily', this.buildAilyStateMarkdown(
      'info',
      `数字孪生组装检测台已开启，请按要求连接以下硬件组件：${components.map((c) => c.displayName).join('、')}`
    ));

    // 016: 检测逻辑已迁移到 Flutter 数字孪生清单面板，通过 postMessage 闭环
    // iframe.component.ts 订阅 state$ 并转发，Flutter 完成后发回 tesseract-assembly-complete
  }

  private async confirmHotplugNodeAndContinue(
    sessionId: string,
    nodeName?: string,
    ...
    try {
      const result = await this.tesseractChatService.confirmNode({
        sessionId,
        nodeName: nodeName || '',
      }, projectPath);
      ...
      // 017: 自动确认剩余硬件节点（排除当前已确认节点）
      const currentNodeName = nodeName || '';
      const remainingNodes = allPendingHardwareNodeNames.filter((n) => n !== currentNodeName);
      let finalResult = result;
      for (const pendingNodeName of remainingNodes) {
        try {
          finalResult = await this.tesseractChatService.confirmNode({
            sessionId,
            nodeName: pendingNodeName,
          }, projectPath);
          if (finalResult?.sessionId) {
            this.chatService.currentSessionId = finalResult.sessionId;
          }
        } catch {
          // 静默跳过：单个节点确认失败不应中断整体流程
        }
      }

      const sideEffects = await this.applyDialogueResponseSideEffects(finalResult?.response);
      this.isWaiting = false;
      this.isCompleted = true;
      this.replaceLastAilyMessage(
        sideEffects.overrideMarkdown || finalResult?.markdown || '硬件配置完成',
        sideEffects.source || 'mainAgent'
      );
      this.saveCurrentSession();
      this.refreshHistoryList();
      ...
      this.replaceLastAilyMessage(`
\`\`\`aily-state
${JSON.stringify({ state: 'error', text: e?.message || '硬件确认失败，请重试' }, null, 2)}
\`\`\`
`);
      return null;
    } finally {
      this.dialogueAssemblyPhase = 'idle';
      this.dialoguePendingComponents = [];
      this.cdr.markForCheck();
    }
  }

  private async maybeTriggerWorkflowDeployAfterAssembly(result: any, data: any): Promise<void> {
    if (data?.triggerWorkflowDeploy !== true) {
      return;
    }

    const responseType = result?.response?.type;
    const sessionId = result?.sessionId || data?.sessionId || this.sessionId;

    if ((responseType === 'summary_ready' || responseType === 'workflow_ready') && sessionId) {
      await this.runTesseractAction('tesseract-confirm-workflow', {
        sessionId,
        autoUploadAfterConfigComplete: true,
      });
      return;
    }

    if (responseType === 'config_complete' && sessionId) {
      await this.runTesseractAction('tesseract-upload-to-hardware', {
        sessionId,
      });
      return;
    }
    ...
    this.appendMessage('aily', this.buildAilyStateMarkdown(
      'info',
      '已返回对话窗口，当前会话会继续推进。若后端进入“下发工作流”阶段，将自动执行对应步骤。'
    ));
    this.saveCurrentSession();
  }
  ...
  menuClick(e) {
    this.openHistorySession(e?.sessionId);
  }
  ...
  switchModel(event: MouseEvent) {
    ...
    if (this.assemblyMonitorSubscription) {
      this.assemblyMonitorSubscription.unsubscribe();
      this.assemblyMonitorSubscription = null;
    }
  ...
  private async switchToMode(mode: string) {
    ...
    // 保存模式到配置
    this.chatService.saveChatMode(mode as 'agent' | 'ask' | 'tesseract');
    // console.log('切换AI模式为:', this.currentMode);
    await this.stopAndCloseSession();
    this.startSession().then((res) => {
      // console.log('新会话已启动，当前模式:', this.currentMode);
    }).catch((err) => {
      this.switchToMode('agent');
    });
  }
  ...
  ngOnDestroy() {
    ...
    if (this.chatActionHandler) {
      document.removeEventListener('aily-chat-action', this.chatActionHandler);
      this.chatActionHandler = null;
    }

    if (this.dialogueBridgeEventSubscription) {
      this.dialogueBridgeEventSubscription.unsubscribe();
      this.dialogueBridgeEventSubscription = null;
    }

    if (this.dialogueBridgeStateSubscription) {
      this.dialogueBridgeStateSubscription.unsubscribe();
      this.dialogueBridgeStateSubscription = null;
    }

    if (this.dialogueHardwareRuntimeSubscription) {
      this.dialogueHardwareRuntimeSubscription.unsubscribe();
      this.dialogueHardwareRuntimeSubscription = null;
    }

    if (this.dialogueSkillsSubscription) {
      this.dialogueSkillsSubscription.unsubscribe();
      this.dialogueSkillsSubscription = null;
    }
  ...
  showSettings = false;
  openSettings(_event?: Event): void {
