/**
 * [INPUT]: 依赖项目/串口/UI 服务、硬件运行时服务、顶部菜单配置、NzModalService 与 SkillCenter 模态组件装配主窗口顶栏交互。
 * [OUTPUT]: 对外提供 HeaderComponent，渲染开发板选择、硬件连接状态框、窗口控制、工具按钮与 Skill Center 入口。
 * [POS]: main-window/components/header 的顶栏控制器，负责把全局项目操作、硬件连接状态框与右上角功能入口收束到一处。
...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  activeSkillCenterView: SkillCenterPrimaryView | null = null;
  ...
  get headerApps(): AppItem[] {
    if (this.router.url.indexOf('/main/tesseract-studio') === -1) {
      return APP_LIST;
    }
  ...
  openSkillCenter(view: SkillCenterPrimaryView): void {
    this.activeSkillCenterView = view;
    ...
    // ---- 隐藏数字孪生窗口，避免遮挡 ----
    window.electronAPI?.digitalTwin?.hide();
    ...
    const modalRef = this.modal.create({
      ...
      nzWidth: 'min(1400px, calc(100vw - 96px))',
      nzStyle: {
        top: '26px',
      },
    ...
    modalRef.afterClose.subscribe(() => {
      // ---- 恢复数字孪生窗口 ----
      window.electronAPI?.digitalTwin?.show();
      ...
      this.activeSkillCenterView = null;
      this.cd.markForCheck();
    });
  }
  ...
  getHardwareConnectionTitle(snapshot: HardwareRuntimeSnapshot | null): string {
