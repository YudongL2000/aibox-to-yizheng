...
import { TestDatabase, TestDataGenerator, isBetterSqlite3Available, getBetterSqlite3Ctor } from './test-utils';

const describeDatabase = isBetterSqlite3Available() ? describe : describe.skip;

const getDatabaseCtor = () => {
...
describeDatabase('Database Connection Management', () => {
    ...
    it('should handle connection pooling simulation', async () => {
      ...
      try {
        for (let i = 0; i < connectionCount; i++) {
          ...
          const conn = new DatabaseCtor(dbPath, { 
            readonly: false,
            fileMustExist: true 
          });
  ...
  describe('Connection Error Handling', () => {
    ...
    it('should handle readonly database access', async () => {
      ...
      // Open as readonly
      const DatabaseCtor = getDatabaseCtor();
      const readonlyDb = new DatabaseCtor(dbPath, { readonly: true });
  ...
  describe('Connection Lifecycle', () => {
    ...
    it('should handle connection timeout simulation', { timeout: 5000 }, async () => {
      ...
      const conn2 = new DatabaseCtor(dbPath);
