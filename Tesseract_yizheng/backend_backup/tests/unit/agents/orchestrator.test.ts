 ...
 * [OUTPUT]: 验证 Orchestrator 的多轮澄清、总结与工作流生成闭环，并锁住 summary->confirm 的最小工作流兜底
...
describe('Orchestrator', () => {
  ...
  it('keeps confirm progressing when summary accepted without explicit capabilities', async () => {
    const { orchestrator, sessionService } = createOrchestrator();
    const session = sessionService.getOrCreate();

    sessionService.setBlueprint(session.id, {
      intentSummary: '用户通过对机器人说话表达情绪',
    ...
    sessionService.setOrchestratorState(session.id, {
      phase: 'reflection',
      decision: 'direct_accept',
      userIntent: '我希望用户通过对机器人说话来表达情绪。',
      capabilityIds: [],
      searchKeywords: ['说话', '情绪'],
      missingFields: [],
      pendingQuestions: [],
      reasoningSummary: '执行动作未明确，但 summary 阶段已经允许先从最小可运行工作流开始。',
      recognizedRequirements: ['目标: 用户通过对机器人说话表达情绪'],
      confidence: 0.61,
      complete: true,
      canProceed: true,
    });
    ...
    expect(response.type).toBe('workflow_ready');
    expect(sessionService.getTraceEvents(session.id).some((event) =>
      event.title === '确认构建回退为最小工作流'
    )).toBe(true);
  });
