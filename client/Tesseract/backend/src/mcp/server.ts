/**
 * [INPUT]: 依赖 MCP SDK、节点仓库、validator、tool definitions 与 n8n handlers
 * [OUTPUT]: 对外提供 N8NDocumentationMCPServer，承接 MCP 工具注册、分发与响应格式
 * [POS]: mcp 模块的主执行器，被 stdio-wrapper 和测试服务复用
...
interface NodeVersionDetailsInfo {
  nodeType: string;
...
type NodeInfoResponse = NodeMinimalInfo | NodeStandardInfo | NodeFullInfo;
...
export class N8NDocumentationMCPServer {
  ...
  async executeTool(name: string, args: any): Promise<any> {
    ...
    switch (name) {
      ...
      case 'get_node':
        ...
        this.assertNoLegacyGetNodeArgs(args);
        return this.getNode(
          args.nodeType,
          args.detail,
      ...
      case 'search_node_properties':
        ...
        return this.searchNodeProperties(
          args.nodeType,
          args.query,
          args.maxResults !== undefined ? Number(args.maxResults) || 20 : 20
        );
      case 'get_node_versions':
        this.validateToolParams(name, args, ['nodeType']);
        return this.getNodeVersions(args.nodeType, args.since, args.toVersion);
      case 'validate_node':
  ...
  private assertNoLegacyGetNodeArgs(args: Record<string, any>): void {
    const legacyArgNames = ['mode', 'propertyQuery', 'maxPropertyResults', 'fromVersion', 'toVersion'];
    const legacyArg = legacyArgNames.find((name) => args[name] !== undefined);

    if (!legacyArg) {
      return;
    }

    const guidanceByArg: Record<string, string> = {
      mode: 'Use get_node_docs, search_node_properties, or get_node_versions instead of mode-based get_node calls.',
      propertyQuery: 'Use search_node_properties({ nodeType, query, maxResults? }) for property lookup.',
      maxPropertyResults: 'Use search_node_properties({ nodeType, query, maxResults? }) for property lookup.',
      fromVersion: 'Use get_node_versions({ nodeType, since, toVersion? }) for version comparisons.',
      toVersion: 'Use get_node_versions({ nodeType, since, toVersion? }) for version comparisons.',
    };

    throw new Error(
      `get_node no longer accepts legacy parameter "${legacyArg}". ${guidanceByArg[legacyArg]}`
    );
  }
...
## Note
Full documentation is being prepared. For now, use get_node({detail: "standard"}) for configuration help.
`,
  ...
  /**
   * Core node information retrieval with progressive detail levels.
   *
   ...
   * @param includeTypeInfo - Include type structure metadata for properties.
   *   Adds ~80-120 tokens per property with type category, JS type, and validation rules.
   * @param includeExamples - Reserved for future configuration examples.
   *   Adds ~200-400 tokens per example.
   * @returns NodeInfoResponse - Node configuration payload for the requested detail level
   */
  ...
  private async getNodeVersions(
    nodeType: string,
    since?: string,
    toVersion?: string
  ): Promise<VersionHistoryInfo | NodeVersionDetailsInfo> {
    await this.ensureInitialized();
    if (!this.repository) {
    ...
    const normalizedType = NodeTypeNormalizer.normalizeToFullForm(nodeType);
    const history = this.getVersionHistory(normalizedType);

    if (!since) {
      return history;
    }

    const latest = this.repository.getLatestNodeVersion(normalizedType);
    const currentHistoryVersion = history.versions.find((version: any) => version.isCurrent)?.version;
    const newestHistoryVersion = history.versions[history.versions.length - 1]?.version;
    const targetVersion =
      toVersion ||
      latest?.version ||
      currentHistoryVersion ||
      newestHistoryVersion;

    if (!targetVersion) {
      return {
        nodeType: normalizedType,
        requestedFromVersion: since,
        targetVersion: 'unknown',
        history,
        message: 'No target version available. Version history exists, but upgrade diff data is unavailable for this node.',
      };
    }
    ...
    return {
      ...
      targetVersion,
      history,
      comparison: this.compareVersions(normalizedType, since, targetVersion),
      breakingChanges: this.getBreakingChanges(normalizedType, since, targetVersion),
      migrations: this.getMigrations(normalizedType, since, targetVersion),
    };
  }
