...
export const n8nAutofixWorkflowDoc: ToolDocumentation = {
  ...
  essentials: {
    ...
    performance: 'Network-dependent (200-1500ms) - fetches, validates, and optionally updates workflow',
    tips: [
      ...
      'Supports expression formats, typeVersion issues, error outputs, and webhook paths',
      'High-confidence fixes (≥90%) are safe for auto-application',
      'Always review fixes before applying to production workflows'
    ]
  },
  full: {
...
4. **Webhook Path Generation**: Automatically generates UUIDs for webhook nodes missing path configuration:
...
Requires N8N_API_URL and N8N_API_KEY environment variables to be configured.`,
    parameters: {
      ...
      fixTypes: {
        type: 'array',
        required: false,
        description: 'Types of fixes to apply. Options: ["expression-format", "typeversion-correction", "error-output-config", "webhook-missing-path"]. Default: all types.'
      },
      confidenceThreshold: {
        type: 'string',
        required: false,
...
- applied: Boolean indicating if fixes were applied (when applyFixes: true)`,
    ...
    performance: 'Depends on workflow size and number of issues. Preview mode: 200-500ms. Apply mode: 500-1500ms for medium workflows.',
    bestPractices: [
