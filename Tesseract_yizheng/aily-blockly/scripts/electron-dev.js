...
const defaultFlutterHost = "127.0.0.1";
const defaultFlutterPort = Number(process.env.TESSERACT_FRONTEND_PORT || 18082);
...
function createElectronEnv(devServerUrl) {
  const desktopTwinMode =
    process.env.TESSERACT_DESKTOP_TWIN_MODE || "flutter-workspace";
  const frontendUrl =
    process.env.AILY_TESSERACT_FRONTEND_URL ||
    process.env.TESSERACT_DESKTOP_TWIN_URL ||
    `http://${defaultFlutterHost}:${defaultFlutterPort}`;
  const env = createChildEnv({
    DEV: "true",
    AILY_BLOCKLY_DEV_SERVER_URL: devServerUrl,
    TESSERACT_DESKTOP_UI_MODE:
      process.env.TESSERACT_DESKTOP_UI_MODE || "single-window",
    TESSERACT_DESKTOP_TWIN_MODE: desktopTwinMode,
    AILY_TESSERACT_FRONTEND_URL: frontendUrl,
  });
...
function shouldUseFlutterWorkspaceTwin() {
  const mode =
    process.env.TESSERACT_DESKTOP_TWIN_MODE || "flutter-workspace";
  return mode === "flutter-workspace";
}

function resolveFrontendTargetUrl() {
  const raw =
    process.env.AILY_TESSERACT_FRONTEND_URL ||
    process.env.TESSERACT_DESKTOP_TWIN_URL ||
    `http://${defaultFlutterHost}:${defaultFlutterPort}`;
...
function spawnFrontendDevServer() {
  const frontendScript = path.join(frontendRoot, "dev_web_start.sh");
  if (!fs.existsSync(frontendScript)) {
    throw new Error(`Missing frontend dev script: ${frontendScript}`);
  }

  const resolvedFlutterBin = resolveFlutterBinary();
  const envPathPrefix = resolvedFlutterBin
    ? `${path.dirname(resolvedFlutterBin)}${path.delimiter}${process.env.PATH || ""}`
    : process.env.PATH || "";
  const env = createChildEnv({
    AGENT_API: process.env.AGENT_API || "http://127.0.0.1:3006",
    FLUTTER_BIN: resolvedFlutterBin || process.env.FLUTTER_BIN || "",
    PATH: envPathPrefix,
  });

  return spawn(frontendScript, [], {
    cwd: frontendRoot,
    stdio: "inherit",
    shell: false,
    env,
...
function resolveFlutterBinary() {
  ...
  const candidates = [
    ...
    process.env.FLUTTER_HOME
      ...
      : "",
    path.join(homeDir, "flutter_sdk_3_41_6", "bin", "flutter"),
    path.join(homeDir, "flutter_sdk", "bin", "flutter"),
  ].filter(Boolean);
...
async function main() {
  ...
  const skipRuntimePrep = process.env.TESSERACT_SKIP_RUNTIME_PREP === "1";
  const skipDesktopAssetsPrep = process.env.TESSERACT_SKIP_DESKTOP_ASSETS === "1";

  if (skipRuntimePrep) {
    console.log("[electron-dev] Skipping prepare:tesseract-runtime (TESSERACT_SKIP_RUNTIME_PREP=1)");
  } else {
    const prepare = spawnNpm(["run", "prepare:tesseract-runtime"]);
    await waitForExit(prepare, "prepare:tesseract-runtime");
  }

  if (skipDesktopAssetsPrep) {
    console.log("[electron-dev] Skipping prepare:desktop-assets (TESSERACT_SKIP_DESKTOP_ASSETS=1)");
  } else {
    const prepareDesktopAssets = spawnNpm(["run", "prepare:desktop-assets"]);
    await waitForExit(prepareDesktopAssets, "prepare:desktop-assets");
  }
  ...
  if (shouldUseFlutterWorkspaceTwin()) {
    const frontendTarget = resolveFrontendTargetUrl();
    console.log(
      `[electron-dev] Using Flutter digital twin workspace at ${frontendTarget.url}`
    );
    console.log(
      `[electron-dev] Waiting up to ${Math.ceil(
        defaultDevServerWaitMs / 1000
      )}s for Flutter digital twin workspace to become reachable`
    );
  ...
  if (frontendProcess) {
    frontendProcess.once("exit", async (code, signal) => {
      if (shuttingDown) {
        return;
      }

      console.error(
        `[electron-dev] Flutter digital twin workspace exited early with ${code ?? "null"}${
          signal ? ` (${signal})` : ""
        }`
      );
      await shutdown(code || 1);
