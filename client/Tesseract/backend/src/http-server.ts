...
function detectBaseUrl(req: express.Request, host: string, port: number): string {
  ...
  const hostHeader = Array.isArray(forwardedHost)
    ? forwardedHost[0]
    : forwardedHost?.split(',')[0] || req.get('host') || `${host}:${port}`;
  return `${protocol}://${hostHeader}`;
}

function getStartupBaseUrl(host: string, port: number): string {
  if (process.env.PUBLIC_URL) return process.env.PUBLIC_URL;
  const protocol = process.env.HTTPS === 'true' ? 'https' : 'http';
  return `${protocol}://${host}:${port}`;
}

function formatEndpointUrls(baseUrl: string) {
  return {
