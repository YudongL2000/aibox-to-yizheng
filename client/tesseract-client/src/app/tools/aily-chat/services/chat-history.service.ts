/**
 * [INPUT]: 依赖 window.fs/path 与 Electron 预注入桥，用于持久化聊天历史并输出 renderer 侧诊断日志。
 * [OUTPUT]: 对外提供 ChatHistoryService、SessionData/SessionIndexEntry 等历史会话存储能力。
 * [POS]: aily-chat 历史会话真相源，统一管理索引、磁盘文件与定时保存。
...
export class ChatHistoryService implements OnDestroy {
  ...
  migrateSessionId(oldId: string, newId: string): void {
    ...
    try {
      ...
      this.emitLog('warn', '[ChatHistory] 迁移数据文件失败（不影响流程）', err);
    }

    // 5. 立即写入索引
    this.writeIndex();
    this.emitLog('info', `[ChatHistory] 会话 ID 已迁移: ${oldId} -> ${newId}`);
  }
    ...
    try {
      ...
      if (this.fileExists(indexPath)) {
        ...
        if (Array.isArray(parsed)) {
          this.index = parsed;
          this.emitLog('debug', `[ChatHistory] 全局索引已加载, ${this.index.length} 条记录`);
        }
      }
    } catch (error) {
      this.emitLog('warn', '[ChatHistory] 加载全局索引失败', error);
      this.index = [];
    }
  }
  ...
  private writeIndex(): void {
    ...
    try {
      ...
      this.emitLog('warn', '[ChatHistory] 写入全局索引失败', error);
    }
  }
  ...
  private writeSessionData(sessionId: string, data: SessionData): void {
    ...
    try {
      ...
      this.emitLog('warn', `[ChatHistory] 写入会话数据失败 (${sessionId})`, error);
    }
  }
  ...
  private readSessionData(sessionId: string, projectPath: string | null): SessionData | null {
    ...
    for (const filePath of paths) {
      try {
        ...
        this.emitLog('warn', `[ChatHistory] 读取会话数据失败 (${filePath})`, error);
      }
    }
  ...
  private startAutoSave(): void {
    this.autoSaveTimer = setInterval(() => {
      if (this.dirtySessionIds.size > 0 || this.indexDirty) {
        this.emitLog(
          'debug',
          `[ChatHistory] 定时保存: ${this.dirtySessionIds.size} 个脏会话, 索引dirty=${this.indexDirty}`
        );
        this.flushAll();
      }
    }, 30000); // 30s
