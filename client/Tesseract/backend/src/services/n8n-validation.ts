...
const workflowNodeNotesSchema = z.union([
  z.string(),
  z
    .object({
      ...
      sub: z.union([z.string(), z.record(z.unknown())]).optional(),
      topology: z.union([z.string(), z.null()]).optional(),
...
export const workflowNodeSchema = z.object({
  ...
  notes: workflowNodeNotesSchema.optional(),
...
export function cleanWorkflowForCreate(workflow: Partial<Workflow>): Partial<Workflow> {
  ...
  if (Array.isArray(cleanedWorkflow.nodes)) {
    cleanedWorkflow.nodes = normalizeNotesForApi(cleanedWorkflow.nodes);
  }
...
function normalizeNotesForApi(nodes: Workflow['nodes']): Workflow['nodes'] {
  return nodes.map((node) => {
    if (node.notes && typeof node.notes === 'object') {
      return {
        ...node,
        notes: JSON.stringify(node.notes),
      };
    }
    return node;
  });
...
export function validateWorkflowStructure(workflow: Partial<Workflow>): string[] {
  ...
  if (workflow.nodes && workflow.nodes.length === 1) {
    ...
    errors.push(`Single-node workflows are invalid for Tesseract. Current node: "${singleNode.name}" (${singleNode.type}). Add another connected processing or execution node before creating the workflow.`);
  }
