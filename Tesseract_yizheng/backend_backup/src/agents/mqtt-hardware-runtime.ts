 ...
 * [OUTPUT]: 对外提供 MQTT 硬件运行时真相源、heartbeat 解析器、command 封装器、publishRecImg rec_img 图片发送、workflow upload/ack 结构化日志、可订阅状态 store 与 canonical digitalTwinScene
...
const HEARTBEAT_DEVICE_ALIAS_PORT_MAP: Record<string, string> = {
  cam1: 'port_3',
  camera1: 'port_3',
  hand1: 'port_4',
  mechanicalhand1: 'port_4',
  mechanicalarm1: 'port_4',
  car1: 'port_1',
  wheel1: 'port_1',
  chassis1: 'port_1',
  screen1: 'port_hdmi',
...
function summarizePublishedPayload(payload: Record<string, unknown>): Record<string, unknown> {
  const msgType = typeof payload.msg_type === 'string' ? payload.msg_type : null;
  const content = Array.isArray(payload.msg_content) ? payload.msg_content : [];
  const first = content[0];

  if (msgType === 'workflow' && first && typeof first === 'object' && !Array.isArray(first)) {
    const workflow = first as Record<string, unknown>;
    return {
      msgType,
  ...
  if (msgType === 'cmd' && first && typeof first === 'object' && !Array.isArray(first)) {
    const command = first as Record<string, unknown>;
    return {
      msgType,
      deviceType: typeof command.device_type === 'string' ? command.device_type : null,
      cmd: typeof command.cmd === 'string' ? command.cmd : null,
    };
  }
...
function summarizeCommandResponse(payload: Record<string, unknown>): Record<string, unknown> {
  return {
    responseType: typeof payload.msg_type === 'string' ? payload.msg_type : null,
    ...
    result:
      typeof payload.result === 'number' || typeof payload.result === 'string'
        ? payload.result
        : null,
    workflowFile: typeof payload.workflow_file === 'string' ? payload.workflow_file : null,
    deviceType: typeof payload.device_type === 'string' ? payload.device_type : null,
    cmd: typeof payload.cmd === 'string' ? payload.cmd : null,
  };
}
...
export class MqttHardwareRuntime extends EventEmitter {
  ...
  private async publishCommand(
    ...
    this.publishStatus();
    logger.info('MqttHardwareRuntime: queueing command', {
      kind: command.kind,
      requestId,
      topic: command.topic,
      ...summarizePublishedPayload(payload),
    });
    ...
    if (!this.transport) {
      ...
      logger.warn('MqttHardwareRuntime: command not published because transport is unavailable', {
        kind: command.kind,
        requestId,
        topic: command.topic,
        enabled: this.enabled,
        resolvedStatus: receipt.status,
      });
      return receipt;
    }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      .then(() => {
        ...
        this.lastCommand = receipt;
        this.publishStatus();
        logger.info('MqttHardwareRuntime: command published', {
          kind: command.kind,
          requestId,
          topic: command.topic,
          ...summarizePublishedPayload(payload),
      ...
      .catch((error) => {
        ...
        this.lastCommand = receipt;
        this.publishStatus();
        logger.warn('MqttHardwareRuntime: command publish failed', {
          kind: command.kind,
          requestId,
          topic: command.topic,
  ...
  private recordCommandResponse(topic: string, rawPayload: string): void {
    ...
    this.publishStatus();
    logger.info('MqttHardwareRuntime: command acknowledged', {
      kind: this.lastCommand.kind,
      requestId: this.lastCommand.requestId,
      topic,
      ...summarizeCommandResponse(payload),
    });
...
function resolveHardwareInterfaceId(
  devicePort?: string,
  devicePath?: string,
  deviceType?: string,
  deviceName?: string,
  ...
  const normalizedPath = normalizeHardwarePortId(devicePath ?? '');
  if (normalizedPath) {
    return normalizedPath;
  }

  return resolveHeartbeatAliasPort(deviceType, deviceName);
}

function resolveHeartbeatAliasPort(deviceType?: string, deviceName?: string): string {
  const aliasCandidates = [deviceName, deviceType]
    .map((value) => String(value || '').trim().toLowerCase())
    .filter((value) => value.length > 0)
    .map((value) => value.replace(/[\s._-]+/g, ''));

  for (const candidate of aliasCandidates) {
    if (HEARTBEAT_DEVICE_ALIAS_PORT_MAP[candidate]) {
      return HEARTBEAT_DEVICE_ALIAS_PORT_MAP[candidate];
    }
  }

  const normalizedComponentId = mapDeviceTypeToComponentId(deviceType ?? '');
  switch (normalizedComponentId) {
    case 'camera':
      return 'port_3';
    case 'hand':
      return 'port_4';
...
function normalizeHeartbeatDevices(devices: unknown[]): ParsedHeartbeatDevice[] {
  ...
  for (const device of devices) {
    ...
    const devicePort = getString(raw, 'device_port') || getString(raw, 'devicePort') || getString(raw, 'port');
    ...
    nextDevices.push({
      ...
      devicePort: devicePort || undefined,
      interfaceId: resolveHardwareInterfaceId(devicePort, devicePath, deviceType, deviceName),
...
function extractStatusPayload(payload: IncomingMqttPayload): IncomingMqttPayload | null {
  ...
  if (payload.msg_type === 'status' && Array.isArray(payload.msg_content)) {
    return {
      ...payload,
      devices: payload.msg_content,
    };
  }
