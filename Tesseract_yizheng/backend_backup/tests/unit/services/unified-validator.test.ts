...
describe('UnifiedValidator', () => {
  ...
  it('classifies single-node workflow issues as needsModel', async () => {
    const validator = new UnifiedValidator({} as any, {
      workflowValidator: {
        validateWorkflow: vi.fn().mockResolvedValue({
          valid: false,
          errors: [{
            type: 'error',
            message: 'Single-node workflows are invalid for Tesseract. Add at least one more connected processing or execution node before returning workflow_ready.',
          }],
          warnings: [],
          suggestions: [],
          statistics: {
