...
class MqttDeviceService {
  ...
  Future<bool> connectAndSubscribe() async {
    ...
    final clientId = '${MqttDeviceConfig.clientId}_${DateTime.now().millisecondsSinceEpoch}';
    print('[MQTT] ========== 开始连接 ==========');
    ...
    print('[MQTT] Path: ${wsPath.isNotEmpty ? wsPath : '(无路径)'}');
    ...
    try {
      ...
      if (conn?.state != MqttConnectionState.connected) {
        ...
        _lastError = errorMsg;
        print('[MQTT] ❌ $errorMsg');
        print('[MQTT] ⚠️ 请确认 broker 已开启 WebSocket（端口 ${MqttDeviceConfig.wsPort}），当前尝试: $wsUrl');
        _isConnecting = false;
        return false;
      }
      ...
      _client!.subscribe(MqttDeviceConfig.topicDeviceEvent, MqttQos.atLeastOnce);
        ...
        for (var msg in list) {
          ...
          if (rec is MqttPublishMessage) {
            ...
            if (kDebugMode && payload.length < 500) {
              print('[MQTT]   内容预览: ${payload.substring(0, payload.length > 200 ? 200 : payload.length)}...');
            }
      ...
      if (e.toString().contains('NoConnectionException')) {
        ...
        print('[MQTT] 当前尝试: ws://${MqttDeviceConfig.host}:${MqttDeviceConfig.wsPort}');
        print('[MQTT] ');
        print('[MQTT] 检测方法：');
        print('[MQTT] 1. 浏览器控制台执行（F12 -> Console）：');
        print('[MQTT]    const ws = new WebSocket("ws://${MqttDeviceConfig.host}:${MqttDeviceConfig.wsPort}");');
