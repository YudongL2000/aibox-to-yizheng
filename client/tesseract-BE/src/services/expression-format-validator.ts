...
export class ExpressionFormatValidator {
  ...
  static validateAndFix(
    ...
    if (invalidResults.length > 0) {
      ...
      if (prefixIssue) {
        // Check if this field should use resource locator format
        const fieldName = fieldPath.split('.').pop() || '';
        const needsResourceLocator = this.shouldUseResourceLocator(fieldName, context.nodeType);
    ...
    if (hasExpression && typeof value === 'string') {
      ...
      if (needsResourceLocator) {
        ...
        return {
          ...
          correctedValue: this.generateCorrection(value, true),
          issueType: 'needs-resource-locator',
          explanation: `Field '${fieldName}' should use resource locator format for better compatibility.`,
          severity: 'warning'
        };
      }
    }
