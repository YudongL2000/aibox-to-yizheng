...
export class TesseractStudioComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  private workspaceFrameDomReady = false;
  ...
  async loadWorkspace(): Promise<void> {
    ...
    this.workspaceFrameDomReady = false;
  ...
  private bindWorkspaceWebview(): void {
    ...
    webview.addEventListener('dom-ready', () => {
      this.workspaceFrameDomReady = true;
  ...
  private async syncWorkspaceFrameUrl(): Promise<void> {
    const webview = this.workspaceFrame?.nativeElement;
    if (!webview || !this.editorUrl || !this.isWorkspaceFrameAttached(webview)) {
      return;
    }

    const currentUrl = String(
      this.workspaceFrameDomReady && typeof webview.getURL === 'function'
        ? webview.getURL()
    ...
    try {
      if (!this.workspaceFrameDomReady) {
  ...
  private isWorkspaceFrameAttached(webview: any): boolean {
    if (!webview) {
      return false;
    }

    if (typeof webview.isConnected === 'boolean') {
